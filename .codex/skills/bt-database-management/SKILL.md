...
7. full rebuild は `bt market-cutover cutover` のまま維持し、sync / J-Quants を行わない retained Market v4 昇格には canonical `bt market-cutover promote-retained` だけを使う。
...
- retained promotion は `bt market-cutover promote-retained` を使い、sync / reset / repair / stock refresh / intraday sync / adjusted-metric materialization / rebuild / J-Quants call を禁止する。成功 report の `noSync: true` と `noJQuants: true` を必ず検証する。
- fresh service からの継続は同一 report ID、retained report ID、backup ID に束縛した dedicated same-attempt recovery を先に行う。lock / journal / staging を手動変更せず、joined failure は exact rollback、unjoined child は両 lease を保持した deferred fencing とする。

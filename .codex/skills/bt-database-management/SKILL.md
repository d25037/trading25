...
7. J-Quants を使って staging を再構築する full rebuild は `bt market-cutover cutover` のまま維持し、既存 retained rehearsal を再構築しない昇格には canonical `bt market-cutover promote-retained REPORT_ID --retained-report-id ... --backup-id ... --symbol ... --strategy ...` だけを使う。

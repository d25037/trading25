...
description: apps/bt で marimo 実験 notebook を追加・改修するときに使うスキル。計算ロジックを src/domains に実装し、runner-first で bundle を作り、notebook は viewer-only で構築・運用する。
---
...
4. `apps/bt/notebooks/playground` の notebook は latest bundle を既定で読む viewer-only にし、fresh recompute は notebook に持ち込まず runner script へ寄せる。

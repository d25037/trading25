...
- パラメータ空間生成、スコアリング、optimize job、HTML 成果物を変更するとき。
...
2. パラメータ空間、strategy YAML 内の `optimization` block、スコア指標の整合を確認する。
3. discovery / validation / walk-forward の境界を確認し、validation 側の将来情報や train 外集計が fast score に混入していないか確認する。
4. 並列実行や timeout 変更時は worker の terminal state を確認する。
5. legacy `*_grid.yaml` は migration 専用経路以外から読まない。新規仕様は strategy YAML トップレベル `optimization` を SoT にする。
6. 成果物（HTML report、metrics JSON、job result payload）の保存規約を崩さない。Notebook artifact 前提を再導入しない。

## Guardrails

- `optimization` job type と backtest family の run contract を壊さない。
- VectorBT の単一 stage と ranked `fast_candidates` の契約を維持する。
- discovery rank や score を計算するときに validation / OOS 由来の bucket、threshold、summary を混ぜない。

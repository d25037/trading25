...
description: bt の最適化エンジン（グリッド探索・スコアリング・可視化）を扱うスキル。optimization domain、worker、`/api/optimize/*` を変更するときに使用する。
---
...
1. route -> service -> worker -> domain の順で責務を確認する。
2. パラメータ空間、スコア指標、verification stage の整合を確認する。
3. 並列実行や timeout 変更時は worker の terminal state を確認する。
4. 成果物（Notebook/HTML、metrics JSON）の保存規約を崩さない。

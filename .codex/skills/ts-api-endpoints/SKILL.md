...
- Type generation scripts: `apps/ts/packages/contracts/scripts/fetch-bt-openapi.ts`
...
3. ts 側呼び出し先 (`/api/*`) と contracts 型 (`@trading25/contracts/types/api-types`, `api-response-types`) の整合を取る。
4. 契約更新時は `bun run --filter @trading25/contracts bt:sync` を実行して型同期する。
5. 生成型の健全性は `bun run --filter @trading25/contracts bt:check` で確認する。

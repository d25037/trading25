...
- Dataset preset SSOT: `apps/ts/packages/contracts/src/types/api-response-types.ts` (`DATASET_PRESETS`)
...
- API 契約変更後は `bun run --filter @trading25/contracts bt:sync` を実行して型を同期する。

## Key Checks

1. `/api/dataset`, `/api/dataset/jobs/{jobId}`, `/api/dataset/{name}/*` の契約整合。
2. `ts/web` が `@trading25/contracts` の shared 型を参照していること。

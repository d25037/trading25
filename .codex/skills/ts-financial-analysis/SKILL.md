...
- `asOfDate`（effective market date）、`provenance.reference_date`（knowledge cutoff）、`priceBasisDate`（basis frontier）の意味をfrontendで再解釈・代替しない。
- Market v5 / `provider_adjusted_v1` の exact provider-window/current-basis response を前提とし、欠損を current/latest 値、frontend-local adjustment、deterministic retryで補完しない。pre-v5/incompatible recovery は `bt market-cutover cutover` の案内だけを表示し、live Initial reset migration を案内しない。

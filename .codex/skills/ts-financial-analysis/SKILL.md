...
1. `bt-openapi.json` で対象エンドポイントの path、query、response を確認する。
   Fundamentalsでは`from` / `to` / `periodType` / `preferConsolidated` / liquidity optionsをそのままforwardし、必須`asOfDate`を保持する。
...
- Market schema v4 / `local_projection_v2_event_time` の exact basis response を前提とし、basis 欠損を current/latest 値、frontend-local adjustment、deterministic retryで補完しない。

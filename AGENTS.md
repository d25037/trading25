...
- retained Market v4 の active 昇格は `bt market-cutover promote-retained REPORT_ID --retained-report-id ... --backup-id ...` のみを canonical contract とする。retained report provenance から source root を解決し、active v3 の create-only immutable backup を command 内で作成・検証して atomic exchange する。source path / force / copy fallback / J-Quants option は持たず、sync / reset / repair / stock refresh / intraday sync / adjusted-metric materialization / rebuild を実行しない。成功 report は `noSync: true` / `noJQuants: true`、exact report/payload/backup/quarantine identity、semantic smoke、server/worker join verdict を必須とする。既存 `bt market-cutover cutover` は explicit full rebuild のまま維持する

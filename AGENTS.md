...
- Market Data Plane は schema v4 / `stock_price_adjustment_mode=local_projection_v2_event_time` のみをサポートする。`stock_data_raw` は raw `O/H/L/C/Vo + AdjFactor` の SoT、`stock_data` は current convenience projection とし、cutoff-aware Fundamentals / liquidity からの参照は禁止する
- `stock_adjustment_bases` と `stock_adjustment_basis_segments` は銘柄ごとの corporate-action regime catalog / factor segments の SoT とする。`basis_id=event-pit-v1:{code}:{valid_from}` の全履歴を保持し、旧basisを最新basis materialization時にpruneしない
...
- `statement_metrics_adjusted` / `daily_valuation` は選択した event-time `basis_id` ごとに保持し、sync / stock refresh / explicit materialization の `adjusted_metrics_pit` stage が構築する。analytics GET/POST はread-onlyで、欠損basisをservice-local再計算・current/latest fallbackで補完しない
...
- schema v3以前または `stock_price_adjustment_mode != local_projection_v2_event_time` の `market.duckdb` は incompatible とし、自動移行・dual read・compatibility aliasを行わない。`initial` sync の `resetBeforeSync=true` で `market-timeseries/market.duckdb` と `market-timeseries/parquet/` を reset してv4再構築するまで `auto` / `incremental` / `repair` / `stocks/refresh` を拒否する
- retained Market v4 の active 昇格は `bt market-cutover promote-retained REPORT_ID --retained-report-id ... --backup-id ... --symbol ... --strategy ...` のみを canonical contract とする。retained report provenance から source root を解決し、active v3 の create-only immutable backup を command 内で作成・検証して atomic exchange する。source path / force / copy fallback / J-Quants option は持たず、sync / reset / repair / stock refresh / intraday sync / adjusted-metric materialization / rebuild を実行しない。成功 report は `noSync: true` / `noJQuants: true`、exact report/payload/backup/quarantine identity、semantic smoke、server/worker join verdict を必須とする。既存 `bt market-cutover cutover` は explicit full rebuild のまま維持する
- retained promotion journal の継続 authorization は process-local とし、fresh service は同一 `REPORT_ID` / retained report ID / backup ID で dedicated same-attempt recovery を先に行う。joined failure は exact rollback、unjoined child は両 lease を保持した deferred fencing とし、operator は lock/journal/staging を手動変更せず ownership 解決後に同一 ID で recovery する。immutable backup と quarantined v3 は成功後も保持し、post-commit cleanup staging は journal に束縛された same-ID recovery だけが完了させる
...
- Dataset snapshot は `dataset.duckdb` と物理ファイル名 `manifest.v2.json`（payload `schemaVersion: 3`）の両方が揃った event-time basis bundle のみ解決対象とし、schemaVersion 2、Market v3以前、旧 `dataset.db` / root-level `*.db` snapshot は unsupported とする
...
- Dataset create（`POST /api/dataset`）は Market v4 `market.duckdb` snapshot を SoT とし、`stock_data_raw` / `stock_master_daily` / `stock_adjustment_bases` / `stock_adjustment_basis_segments` / `statements` / 全basisの `statement_metrics_adjusted` / `daily_valuation` / `margin_data` / `topix_data` / `indices_data` を DuckDB 直結 copy で取り込む。4桁/5桁 codeを正規化し、event-time lineageを検証する。生成物は `dataset.duckdb + parquet/ + manifest.v2.json`（payload `schemaVersion: 3`）。public API route は `market_reader` と `market.duckdb` source を必須とし、source 不在時は作成不可にする。再試行は `overwrite=true` に統一し、`resume` route と `timeoutMinutes` request override は廃止した。backend job timeout は内部固定値のみを使う

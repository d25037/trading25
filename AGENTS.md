...
- schema v4以前または `stock_price_adjustment_mode != provider_adjusted_v1` の `market.duckdb` は incompatible とし、in-place migration・自動移行・dual read・compatibility aliasを行わない。Market v5 へは `bt market-cutover cutover` の isolated full rebuild only（唯一の経路）を使う
- retained Market v4 は v5 candidate として ineligible であり、rehearsal再利用・rebuild・active昇格を行わない。cutover は Market v5 `providerVintage` + Dataset `schemaVersion: 4` semantic smoke、immutable backup、atomic activation、exact rollback を必須とする
- cutover report / journal / backup / staging / active / retained-runtime / quarantine identity は `operations/market-v5-cutover` と active Market root の exact evidence のみを使う。durable state は `prepared` → `exchange_started` → `activated` → `reported`。fresh service は新規 preparation より先に同一 report/rehearsal/backup ID・config・code identity に束縛した exact same-ID recovery を行う。failure artifact、retained-runtime、quarantined tree、immutable backup は保持し、operator は journal / operation lock / staging / backup / active / retained-runtime / quarantine を手動変更せず、unjoined child の ownership 解決前に rollback を強行しない
...
- `/api/analytics/fundamental-ranking` は `market.duckdb`（`daily_valuation`/`statement_metrics_adjusted`/`stocks`/`stock_data`）を SoT とし、`metricKey` と `rankings.ratioHigh` / `rankings.ratioLow` を返す。現在の `metricKey` は `eps_forecast_to_actual`（最新の予想EPS / 最新の実績EPS）で、`daily_valuation.forward_eps / daily_valuation.eps` を採用する。`forecastAboveRecentFyActuals=true` と `forecastLookbackFyCount` で「最新予想EPS > 直近FY X回の実績EPS最大値」条件を追加フィルタできる。将来の比率指標追加は `metricKey` で識別する

...
この repository は個人開発・個人運用を前提とする。repo 構造、作業手順、検証方針の判断では、堅牢性の最大化より開発速度・運用速度の高速化を優先し、個人運用に不要な複雑化や儀式的手順を増やさない。ただし、データ破壊、秘密情報漏えい、回復困難な外部変更を防ぐ最低限の安全境界は維持する。
...
- Market Data Plane は schema v5 / `stock_price_adjustment_mode=provider_adjusted_v1` のみをサポートする。`stock_data_raw` は raw `O/H/L/C/Vo/Va + AdjFactor + AdjO/AdjH/AdjL/AdjC/AdjVo` の provider provenance SoT、`stock_data` は provider `Adj*` と完全一致する consumer SoT とする
- `stock_provider_windows` は銘柄ごとの provider plan / as-of / effective coverage / source fingerprint、`stock_adjustment_events` は non-unit factor event ledger の SoT とする。`stock_adjustment_bases` / `stock_adjustment_basis_segments` は Market v5 では unsupported
- `statements` は J-Quants raw fundamentals provenance の SoT とし、consumer-facing な adjusted per-share 指標は `statement_metrics_adjusted`、PER/PBR/forward PER は `daily_valuation` を SoT とする。official EPS/BPS history を明示表示する場合のみ raw `statements` fields を使ってよい
- `statement_metrics_adjusted` は current provider basis の disclosure identity ごとに1行を保持し、`daily_valuation` は provider-adjusted price と disclosure PIT ASOF join の SoT とする。normal sync が changed statement / affected code のみ再構築し、standalone materialization や service-local/current/latest fallback は行わない
- `stock_data_minute_raw` は `/equities/bars/minute` 由来の raw `O/H/L/C/Vo/Va` の SoT とし、daily `stock_data` / `topix_data` と混ぜない
- `POST /api/db/sync` の `dataPlane` override は `backend=duckdb-parquet` のみ受け付ける
- `GET /api/db/stats` / `GET /api/db/validate` の時系列スナップショット SoT は DuckDB inspection（`timeSeriesSource` を返却）で、`margin_data` と `options_225_data` の件数・日付範囲・diagnostics、`stock_data_minute_raw` の件数・日付範囲・freshness もここから返す。`/api/db/validate` は deferred な fundamentals/margin empty-response negative cache を info 扱いに留め、TOPIX に対して `options_225_data` が未投入・stale・partial coverage の場合は actionable warning として返す
...
- `/api/db/sync` は `initial` / `incremental` の2モードだけを受け付け、既定は `incremental` とする。`initial` は `resetBeforeSync=true` を必須とし、既存 `market.duckdb` / `parquet/` を削除して再構築する。両モードは `Bulk+REST hybrid` を stage 単位で選択し、予測外部request数が最小の手法を優先する。`stock_data` stage は `enforceBulkForStockData=true` の場合に bulk 必須とし、bulk 不可時は REST fallback せず理由付きでエラー終了する。incremental `stock_data` は TOPIX の新規日付だけでなく DuckDB inspection の `missing_stock_dates` も backfill 対象に含める。`margin_data` stage も `/markets/margin-interest` で bulk/rest を選択し、既存 `market.duckdb` に margin snapshot が無い場合は `incremental` でも full backfill する。`options_225_data` stage は `/derivatives/bars/daily/options/225` を日付単位で同期し、`initial` は `topix_data` 日付集合を基準に backfill、`incremental` は latest options anchor 以降を対象にするが、options anchor が無く local TOPIX がある場合は full local `topix_data` 日付集合で backfill する。margin backfill 対象は current listed markets（`0111/0112/0113`）のみとし、empty response は trading frontier 単位の negative cache として suppress する。fundamentals backfill は preferred-share listed code を親 issuer code へ canonicalize し、empty response は disclosed frontier 単位の negative cache として suppress する。stock ingest は raw `O/H/L/C/Vo/Va + AdjFactor + AdjO/AdjH/AdjL/AdjC/AdjVo` を `stock_data_raw` に保存し、provider-adjusted `Adj*` と完全一致する `stock_data` を同一 per-code window transaction で publish する。`incremental` で DuckDB inspection の `topix/stock/indices` が空かつアンカー不在なら cold-start bootstrap を選び、全日付 fallback 暴走を回避する。listed-market fundamentals や price/options gap など recoverable な不足も `incremental` で回復する。`stocksNeedingRefresh` は互換フィールドとして残すが常時 0 / empty を返す
- `POST /api/db/intraday/sync` と `uv run bt intraday-sync` は `/equities/bars/minute` を bulk/rest で取り込み、`stock_data_minute_raw` に publish する。minute freshness は `LAST_INTRADAY_SYNC` と local minute snapshot を SoT とし、現状は `16:45 JST` cutoff の wall-clock policy を使う
...
- schema v4以前または `stock_price_adjustment_mode != provider_adjusted_v1` の `market.duckdb` は incompatible とし、in-place migration・dual read・compatibility aliasを行わない
- `/api/db/sync` と `POST /api/db/stocks/refresh` の時系列アンカー判定・publish/index は DuckDB inspection + time-series store を必須 SoT とし、旧 SQLite 時系列テーブルへの fallback を禁止する（inspection 失敗時はエラーで停止）。`POST /api/db/stocks/refresh` は provider window 全pageをfetch/validate後、raw + provider-adjusted rowsをcode単位の同一transactionで置換する
...
- Dataset snapshot は `dataset.duckdb` と物理ファイル名 `manifest.v2.json`（payload `schemaVersion: 4`）の両方が揃った Market v5 provider-vintage bundle のみ解決対象とし、schemaVersion 3以前、Market v4以前、旧 `dataset.db` / root-level `*.db` snapshot は unsupported とする
...
- Dataset create（`POST /api/dataset`）は Market v5 `market.duckdb` snapshot を SoT とし、effective provider coverage内の `stock_data_raw` / `stock_master_daily` / `statements` / current-basis `statement_metrics_adjusted` / `daily_valuation` / `margin_data` / `topix_data` / `indices_data` を DuckDB 直結 copy で取り込む。provider plan / as-of / coverage / source fingerprint / fundamentals basis dateを固定し、生成物は `dataset.duckdb + parquet/ + manifest.v2.json`（payload `schemaVersion: 4`）。public API route は `market_reader` と `market.duckdb` source を必須とし、source 不在時は作成不可にする。再試行は `overwrite=true` に統一し、`resume` route と `timeoutMinutes` request override は廃止した。backend job timeout は内部固定値のみを使う
...
- statements の share-adjusted per-share 指標（EPS/BPS/配当/forecast）は `statement_metrics_adjusted` を共通SoTとし、valuation は `daily_valuation` を使う。split / reverse split 調整は `stock_data_raw.adjustment_factor` 由来に限定し、`shares_outstanding` 変化だけでは分割扱いしない。raw `statements` は official history / provenance 用に保持する
- Fundamentals の `POST /api/fundamentals/compute` と `GET /api/analytics/fundamentals/{symbol}` は同一のMarket v5 current-basis PIT snapshot reader・日付検証・error mappingを使う。`to` はknowledge cutoff、`from` はresponse display lower boundで、ISO `YYYY-MM-DD`を厳格検証し `from > to` は422。response `asOfDate`はeffective market date、`provenance.reference_date`はknowledge cutoffを返す。未上場は404、provider window/current-basis/master欠損・不整合は409、開示なしは200 empty dataとする
...
- Fundamentals は EPS に加えて `dividend_fy` / `forecast_dividend_fy` と `payout_ratio` / `forecast_payout_ratio`（実績/予想）を SoT とし、Symbol Workbench の Fundamentals panel と Backtest Signal system（`forward_dividend_growth` / `dividend_per_share_growth` / `payout_ratio` / `forward_payout_ratio`）で同一指標を使う。配当性向は API 返却時に percent 単位へ正規化し、decimal スケール値（例: 0.283）を 28.3% として扱う
- fundamentals 最新値の forecast EPS は同一期末内で `DiscDate` が新しい開示を優先し、旧開示値の逆転表示を防ぐ
- Symbol Workbench Fundamentals panel は `forecastEpsAboveRecentFyActuals`（最新予想EPS > 直近FY X回の実績EPS最大値）を latest metrics で返し、`forecastEpsLookbackFyCount` に応じた true/false を表示する
- `/api/analytics/fundamental-ranking` は `market.duckdb`（`daily_valuation`/`statement_metrics_adjusted`/`stocks`/`stock_data`）を SoT とし、`metricKey` と互換用 `rankings.ratioHigh` / `rankings.ratioLow` に加えて `forecastHigh` / `forecastLow` / `actualHigh` / `actualLow` を返す。現在の `metricKey` は `eps_forecast_to_actual`（最新の予想EPS / 最新の実績EPS）で、`daily_valuation.forward_eps / daily_valuation.eps` を採用する。Ranking item と Symbol Workbench `latestMetrics` の actual EPS / forecast EPS / ratio / change rate は `domains/fundamentals/eps_metric_snapshot.py` の共通契約を使い、同じ TOPIX effective session と current-basis batch validatorで解決する。`forecastAboveRecentFyActuals=true` と `forecastLookbackFyCount` で「最新予想EPS > 直近FY X回の実績EPS最大値」条件を追加フィルタできる。将来の比率指標追加は `metricKey` で識別する
...
- web `Market DB` ページは DuckDB SoT 同期 + DuckDB Snapshot 表示を提供し、`enforceBulkForStockData` を UI で切り替え可能とする。同期操作は既定の `Incremental` と破壊的な `Initial RESET` の2つだけを表示する。`Initial RESET` は必ず typed `RESET` confirm dialog を経て `resetBeforeSync=true` を送り、`market.duckdb` / `parquet/` を削除して再同期する。任意 reset switch、reset eligibility、専用 warning-recovery action は持たず、recoverable な validation warning は `Incremental` を導線とする。`/api/db/validate` の diagnostics は actionable warning と informational diagnostics を分離し、preferred-share issuer alias coverage、negative-cache sample codes、`options_225_data` の missing/stale/partial local coverage と UnderPx missing/conflict diagnostics を表示する。sync中は `GET /api/db/sync/jobs/{jobId}/fetch-details` をポーリングして stageごとの fetch strategy/execution（bulk/rest, endpoint, reason/fallback）を表示する。active sync job は `localStorage + /api/db/sync/jobs/active` で再読み込み/再訪時も復元し、running中は snapshot (`/api/db/stats` `/api/db/validate`) を短周期ポーリング + 進捗更新トリガで自動再取得する

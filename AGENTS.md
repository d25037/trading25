...
- Dataset create（`POST /api/dataset`）は `market.duckdb` snapshot を SoT とし、`indices_data`（sector index catalog: `0040-0060` / `0080-0090`）を含めて batch copy で取り込む。`stock_data` は `build_stock_data_row` で欠損OHLCV行をスキップして warning 集約を返す。再試行は `overwrite=true` に統一し、`resume` route と `timeoutMinutes` request override は廃止した。backend job timeout は内部固定値のみを使う

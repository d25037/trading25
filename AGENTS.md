...
- legacy `market.duckdb`（`stock_price_adjustment_mode` 無し、または `stock_data_raw` 不在で `stock_data` のみ存在）は incompatible とし、`market-timeseries/market.duckdb` と `market-timeseries/parquet/` を reset して `initial sync` をやり直すまで `auto` / `incremental` / `repair` / `stocks/refresh` を拒否する

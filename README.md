...
- `apps/ts` - Bun workspace（web / contracts / utils / api-clients）
...
- `issues` - GitHub Issues移行前のローカルアーカイブ（active taskはGitHub Issuesで管理）
- `scripts` - ルート統合スクリプト
...
- 同期モードは既定の `incremental` と破壊的な `initial` の2つだけ。`initial` は必ず typed `RESET` 確認後に `resetBeforeSync=true` で実行し、既存の `market.duckdb` / `parquet/` を削除して再構築する
- listed-market fundamentals、price、options の recoverable な不足は `incremental` で回復する。validation では missing と unsupported/empty を分離表示し、preferred-share issuer alias coverage と negative-cache sample を確認可能
...
`bt:sync` は `apps/bt` のソースから OpenAPI を直接 export し、その成功後に
snapshot と TypeScript 型を更新します。source export が失敗した場合はエラー終了し、
起動中サーバーや既存の stale snapshot へフォールバックしません。既存 snapshot だけを
使って型を再生成する明示的な offline path は `bt:generate-offline` です。

...
- `repair` sync mode と `Warning Recovery` カードは、listed-market fundamentals backfill など非price warningの回復専用。price/options gapは`incremental` syncで回復する。validation では missing と unsupported/empty を分離表示し、preferred-share issuer alias coverage と negative-cache sample を確認可能

...
入力 panel は `/tmp/trading25-research/market-behavior/annual-first-open-last-close-fundamental-panel/20260501_prime_standard_check/`。`PBR > 0` と `forward PER > 0` を要求した positive-ratio run を使い、`32,264` realized events から `21,532` analysis / scored events を作った。独立効果は `annual-fundamental-confounder-analysis/20260501_prime_standard_check_positive` の `panel_regression_df`、portfolio lens は `annual-value-composite-selection/20260501_prime_standard_check_positive` の `portfolio_summary_df` を参照する。
...
| --- | ---: | ---: | ---: | --- |
| `prime` | `+4.53pp / t=7.00` | `+2.74pp / t=6.49` | `+3.60pp / t=9.24` | 小型と低 `forward PER` が主役。低 `PBR` も正だが相対的に薄い。 |
| `standard` | `+2.70pp / t=5.44` | `+5.41pp / t=9.30` | `+3.14pp / t=6.04` | 低 `PBR` が最も強い。低 `forward PER` と小型も残る。 |
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `prime` | `equal_weight` | `10%` | `1,256` | `22.08%` | `1.23` | `-33.25%` |
| `prime` | `fixed_55_25_20` | `10%` | `1,256` | `22.97%` | `1.28` | `-32.71%` |
| `prime` | `walkforward_regression_weight` | `10%` | `1,256` | `23.06%` | `1.27` | `-32.78%` |
| `standard` | `equal_weight` | `10%` | `722` | `35.49%` | `2.13` | `-30.28%` |
| `standard` | `fixed_55_25_20` | `10%` | `722` | `35.84%` | `2.13` | `-30.78%` |
| `standard` | `walkforward_regression_weight` | `10%` | `722` | `35.75%` | `2.14` | `-31.05%` |
...
Standard は top-decile return / Sharpe が高く、低 `PBR` の独立効果も最も強いため、PBR tilt を primary profile にする。Prime は Standard より return 水準は低いが、独立効果では小型と低 `forward PER` が強く、portfolio lens でも top-decile は成立している。Prime / Standard を単一ランキングに混ぜると、return profile と capacity profile が混在して解釈しにくいため、market-specific sleeve として分ける方が自然。
...
- Input panel: `/tmp/trading25-research/market-behavior/annual-first-open-last-close-fundamental-panel/20260501_prime_standard_check/`
- Independent factor bundle: `/tmp/trading25-research/market-behavior/annual-fundamental-confounder-analysis/20260501_prime_standard_check_positive/`
- Portfolio lens bundle: `/tmp/trading25-research/market-behavior/annual-value-composite-selection/20260501_prime_standard_check_positive/`
...
- Source bundles:
  - `annual-fundamental-confounder-analysis/20260501_prime_standard_check_positive`
  - `annual-value-composite-selection/20260501_prime_standard_check_positive`
- Product surface:

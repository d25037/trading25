...
この readout は bundle `/private/tmp/trading25-research/market-behavior/earnings-holdthrough-expectancy/20260516_earnings_holdthrough_liquidity_z_v2` に基づく。入力 DB は `~/.local/share/trading25/market-timeseries/market.duckdb`、対象は `2016-04-01` から `2026-05-14`。
...
2026-05-16 の `overheat + adjusted valuation SoT` rerun で、Daily Ranking と同じ `20d return >= +30%` を `overheat_state` として追加した。決算またぎでは Prime FY の `overheat` は件数が少ないが、20d TOPIX excess median `-2.39%`、severe loss `28.10%` で、従来の `20d strong_runup` avoid 判断を補強する。Prime non-FY は mean が `+5.41%` と右尾はあるが、median は `-0.32%`、severe loss は `27.87%` で、risk cap なしに買う bucket ではない。

| scope | overheat state | events | positive rate | negative rate | mean 20d excess | median 20d excess | severe loss |
|---|---|---:|---:|---:|---:|---:|---:|
| Prime FY | `not_overheat` | `24,488` | `50.68%` | `27.74%` | `+0.02%` | `-1.08%` | `14.71%` |
| Prime FY | `overheat` | `210` | `41.90%` | `19.52%` | `+1.03%` | `-2.39%` | `28.10%` |
| Prime non-FY | `not_overheat` | `54,246` | `20.79%` | `16.24%` | `+0.09%` | `-0.73%` | `12.07%` |
| Prime non-FY | `overheat` | `244` | `22.95%` | `13.11%` | `+5.41%` | `-0.32%` | `27.87%` |
...
| 用途 | 候補 |
|---|---|
| Avoid / risk cap | Prime FY の `20d strong_runup`、`20d return >= +30%` (`overheat`)、`20d runup x 60d runup x ADV/FF 0.1-0.5` |
...
- latest result bundle: `~/.local/share/trading25/research/market-behavior/earnings-holdthrough-expectancy/20260516_overheat_adjusted_sot`

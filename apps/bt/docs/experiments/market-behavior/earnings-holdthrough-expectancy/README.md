...
この readout は bundle `/private/tmp/trading25-research/market-behavior/earnings-holdthrough-expectancy/20260515_earnings_holdthrough_exante_v2` に基づく。入力 DB は `/Users/shinjiroaso/.local/share/trading25/market-timeseries/market.duckdb`、対象は `2016-04-01` から `2026-05-14`。
...
- `Med ADV60` は Daily Ranking と同じ trailing median `close * volume` を使うが、Phase 1 の liquidity regime は `ADV60 / free-float` と 20d/60d pre return の単純分類で、Daily Ranking の Prime cross-section residual z そのものではない。
...
- latest result bundle: `/private/tmp/trading25-research/market-behavior/earnings-holdthrough-expectancy/20260515_earnings_holdthrough_exante_v2`
...
uv run --project apps/bt python apps/bt/scripts/research/run_earnings_holdthrough_expectancy.py \
  ...
  --run-id 20260515_earnings_holdthrough_exante_v2
```
...
- `precondition_outcome_df`: ex-ante grouping table. Groups by market / FY / pre 20d and 60d return bucket / ADV60-to-free-float bucket, then reports event outcome rates and forward returns.

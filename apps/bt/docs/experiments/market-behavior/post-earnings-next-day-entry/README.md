...
この readout は bundle `/private/tmp/trading25-research/market-behavior/post-earnings-next-day-entry/20260516_post_earnings_next_day_liquidity_z_v2` に基づく。入力 DB は `/Users/shinjiroaso/.local/share/trading25/market-timeseries/market.duckdb`、対象は `2016-04-01` から `2026-05-14`。
...
2026-05-16 follow-up で、発表前営業日 `pre_event_date` 時点の Prime `liquidity_residual_z` と `liquidity_regime` を追加した。`20d strong_runup` を除外した Prime FY post-entry では、`stale_liquidity` と missing が弱く、high residual state が強い。

| state | events | mean 20d excess | median 20d excess | win rate | severe loss |
|---|---:|---:|---:|---:|---:|
| `stale_liquidity` | `2,862` | `-1.38%` | `-1.86%` | `36.34%` | `8.00%` |
| `neutral` | `14,772` | `-0.27%` | `-1.01%` | `41.97%` | `9.36%` |
| `distribution_stress` | `1,980` | `+1.07%` | `-0.16%` | `47.22%` | `12.78%` |
| `rerating_participation` | `906` | `+1.52%` | `+0.56%` | `50.22%` | `12.03%` |
| `missing` | `5,448` | `-0.91%` | `-1.61%` | `39.92%` | `10.74%` |

`forward PER <= 10` と組むと state の差はさらに明確になる。Prime FY / `forward PER <= 10` / `20d strong_runup` 除外では、`stale_liquidity` が median `-1.93%`、`rerating_participation` が median `+0.56%`。低 forward PER 単独より、流動性 state を合わせた方が entry 条件として読みやすい。

#### Overheat Follow-Up

2026-05-16 の `overheat + adjusted valuation SoT` rerun で、post-entry 側にも `20d return >= +30%` の `overheat_state` を追加した。発表後に positive event を確認してから入っても、Prime FY `overheat` は 20d severe loss が `21.37%` まで上がり、not-overheat の `9.54%` よりかなり重い。non-FY positive `overheat` は 5d median `+4.39%` と右尾があるが、sample は `61` events で、20d median は `-1.55%`。

| scope | event | overheat state | executable | 5d median | 20d median | 20d mean | severe loss |
|---|---|---|---:|---:|---:|---:|---:|
| Prime FY | positive | `not_overheat` | `13,041` | `-0.32%` | `-0.86%` | `-0.01%` | `9.54%` |
| Prime FY | positive | `overheat` | `117` | `-0.20%` | `-1.13%` | `+1.57%` | `21.37%` |
| Prime non-FY | positive | `not_overheat` | `11,270` | `-0.36%` | `-0.02%` | `+0.50%` | `8.82%` |
| Prime non-FY | positive | `overheat` | `61` | `+4.39%` | `-1.55%` | `+6.98%` | `18.03%` |
...
| Avoid / sizing | `ADV/FF >= 2.0%`、`strong_runup`、`20d return >= +30%` (`overheat`)、`limit_up_no_fill` 近傍 |
...
- latest result bundle: `/Users/shinjiroaso/.local/share/trading25/research/market-behavior/post-earnings-next-day-entry/20260516_overheat_adjusted_sot`
...
uv run --project apps/bt python apps/bt/scripts/research/run_post_earnings_next_day_entry.py \
  ...
  --run-id 20260516_post_earnings_next_day_liquidity_z_v2
```

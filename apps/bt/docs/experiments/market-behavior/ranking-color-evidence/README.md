...
| `流動性Z` | green は3条件に限定: `neutral_rerating` は low PER20 + Fwd PER/PER <= 0.8 のみ green、`crowded_rerating` は low PBR20 + low Fwd PER20 または low PER20 + Fwd PER/PER <= 0.8 のみ green。`neutral_rerating` は green 以外を一括 blue にせず、low PBR20 + low Fwd PER20 を blue、low PBR20 only または low PER20 + Fwd PER/PER <= 1.0 を light blue、value confirmation なしを gray/neutral とする。`crowded_rerating` は中valueあり blue・valueなし yellow。ただし `crowded_rerating` は PER/Fwd PER/Fwd P/OP/PBR のいずれかが high 20%、または PER/Fwd PER が両方 null（赤字・正のPER未成立を含む）なら、green 条件に該当しない限り yellow。`distribution_stress` / `stale_liquidity` は yellow。120D/150D は色そのものではなく、`crowded_rerating green` に限って `120d <= 0` caution overlay として扱う |

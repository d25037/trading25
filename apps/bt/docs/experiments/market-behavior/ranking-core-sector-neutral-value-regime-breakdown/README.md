...
Daily Ranking の年次factor breakdownを、raw `Undervalued` ではなく同日・同市場・同33業種内の sector-neutral value percentile でやり直す研究。目的は、2022年以降に銀行業へ集中していた `Momentum Value + Balanced Sector Strength: Strong` のreturnを、他セクターの sector-neutral value で置換できるかを確認すること。

## Published Readout

### Decision

今回の結論は、sector-neutral value は捨てるべきではないが、現行の `Momentum Value + Balanced Sector Strength: Strong` を単純に sector-neutral value へ置き換えても、2022年以降の銀行依存returnはまだ他セクターへ十分には移せていない、というもの。

2022-2026 の 20D では、`Sector-Neutral Momentum Value + Balanced Sector Strength: Strong` は銀行業比率を下げる一方、raw `Momentum Value + Balanced Sector Strength: Strong` に年次medianで劣後する。ex Banks に限定しても、sector-neutral版がraw版を安定して上回る状態ではない。2026 の 20D ex Banks だけは改善するが、標本は小さく、60Dではまだ弱い。
...
Production implication は、raw `Momentum Value + Balanced Sector Strength: Strong` を sector-neutral value に置換しないこと。次に進めるなら、単純な daily association ではなく、sector cap / sector-balanced portfolio / ex Banks positive gate を入れた portfolio lens で、銀行業以外からreturnを取れるかを再検証する。
...
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |
| 20D | All sectors | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | -2.28% | 0 | 5 | 46.8% | 77.2% |
| 20D | ex Banks | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | -1.77% | 1 | 5 | 0.0% | 0.0% |
| 20D | All sectors | Hybrid vs raw `Momentum Value + Balanced Sector Strength: Strong` | -1.03% | 1 | 5 | 56.4% | 77.2% |
| 20D | ex Banks | Hybrid vs raw `Momentum Value + Balanced Sector Strength: Strong` | -0.29% | 2 | 5 | 0.0% | 0.0% |
| 60D | All sectors | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | +0.02% | 3 | 5 | 47.8% | 78.1% |
| 60D | ex Banks | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | -0.73% | 1 | 5 | 0.0% | 0.0% |
...
| Year | Scope | Raw `Momentum Value + Balanced Sector Strength: Strong` | Sector-neutral | Hybrid | Raw bank share | Sector-neutral bank share |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
...
| 20D | All sectors | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | +0.44% | 4 | 6 | 6.1% | 20.6% |
| 20D | ex Banks | Sector-neutral vs raw `Momentum Value + Balanced Sector Strength: Strong` | +0.17% | 3 | 6 | 0.0% | 0.0% |
...
ただし、`Balanced Sector Strength: Strong` を重ねた 60D では sector-neutral版がrawを下回る。したがって「sector-neutral value + sector strength」が一貫して良いのではなく、regimeやhorizonで効き方が変わる。

#### 結論: NT regime別では、2016-2021 Flatの損失は軽くなるが、2022-2025の銀行相場は置換できない

20D TOPIX excess median。

| Period | NT regime | Raw `Momentum Value + Balanced Sector Strength: Strong` | Sector-neutral | Hybrid | Raw bank share | Sector-neutral bank share |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
...
2022-2026、20D、`Momentum Value + Balanced Sector Strength: Strong` 系のbreadth regime別集計。
...
銀行業が強かった 2022年以降は、raw `Momentum Value + Balanced Sector Strength: Strong` のheadline returnが銀行業の寄与を大きく含む。sector-neutral化すると銀行比率は下がるが、他セクターがそのreturnを埋めきれていない。これは「銀行業に依存していたreturnを他セクターから得る」という今回の中核仮説に対して、現行の単純な sector-neutral value 定義だけでは不十分、という結果。
...
| --- | --- |
| Raw `Momentum Value + Balanced Sector Strength: Strong` | 2022年以降のheadlineは銀行業依存が強く、cross-sector strategyとしては採用しない |
| `Sector-Neutral Momentum Value + Balanced Sector Strength: Strong` | 銀行比率は下がるが、2022-2026のnon-bank置換には失敗。単純置換はしない |
| `Hybrid Momentum Value + Balanced Sector Strength: Strong` | rawより銀行比率を下げるが、20Dではrawに劣後。60D all-sectorは互角に近いがex Banksでは弱い |
...
- `Balanced Sector Strength` は Daily Ranking SoT通り、公式33業種指数scoreと構成銘柄scoreの平均を前提にしたsector overlay。

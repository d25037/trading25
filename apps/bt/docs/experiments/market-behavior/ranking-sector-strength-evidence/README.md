...
2026-06-01 update `20260601_ranking_sector_strength_average_score_v5` で、`sector_strength_score` は「公式33業種指数 score」と「構成銘柄 score」の平均を採用する。公式指数 score は `official sector index` の 5D / 20D / 60D TOPIX excess rank を `0.20 / 0.45 / 0.25`、構成銘柄 20D breadth rank を `0.10` で合成する。構成銘柄 score は構成銘柄の 20D / 60D TOPIX excess rank と 20D breadth rank の平均。旧scoreでサービス業は最新 `2026-05-29` に `0.29` 近辺だったが、これは `0060` 公式指数ではなく Prime 構成銘柄の等ウェイト平均が弱かったため。公式指数のみではサービス業 `9050` / index `0060` は score `0.80` の `sector_strong` になる一方、構成銘柄側は `0.28125` なので、採用scoreは平均 `0.540625` の `sector_neutral` とする。これはチャート上の sector beta を診断列に残しつつ、構成銘柄の広がりが弱い局面を過剰に `strong` 扱いしないため。

#### 結論: average score を production の trade score として採用する

同一条件の 20D outcome 比較では、平均scoreは buy 側を index-only と同等以上に保ちつつ、short 側で index-only の弱さを避けた。`buy strong` は `rerating blue/green + strong sector`、`short weak` は bad valuation crowded bucket + `sector_weak`。

| Score method | Buy strong obs | Buy median | Buy win rate | Short weak obs | Short excess median | Short excess negative | Short raw median | Read |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| constituent-only | 11,714 | +1.813% | 61.70% | 9,767 | -3.631% | 65.05% | -3.333% | short は強いが、サービス業のような公式指数主導のsector betaを過小評価 |
| index-only | 7,950 | +2.003% | 62.81% | 8,016 | -1.659% | 56.72% | -1.379% | buy は改善するが、売り側の weak sector filter が鈍る |
| average adopted | 9,290 | +2.014% | 62.82% | 6,996 | -3.253% | 63.78% | -2.925% | buy / short の両方で最もバランスが良い |

#### 旧index-based score の参考

`color_sector_interaction_df`、2026-06-01 v4、20D TOPIX excess。

| Bucket | Sector | Obs | Median | Win rate | Severe loss | Read |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `neutral_rerating blue + low_pbr20_low_fwd_per20` | `sector_strong` | 7,614 | +2.055% | 63.63% | 2.72% | 本命blue。公式sector index tailwind が効く |
| `neutral_rerating blue + low_pbr20_low_fwd_per20` | `sector_neutral` | 26,640 | +0.851% to +1.267% | 56.12% to 59.19% | 2.60% to 3.28% | 通常候補 |
| `neutral_rerating blue + low_pbr20_low_fwd_per20` | `sector_weak` | 1,825 | +0.836% | 57.70% | 1.75% | hard exclude ではない |
| `neutral_rerating blue + low_pbr20_only` | `sector_strong` | 8,638 | +0.764% | 54.51% | 4.78% | medium value は sector tailwind で補強 |
| `neutral_rerating blue + no_value_confirmation` | `sector_strong` | 44,594 | -1.007% | 43.88% | 9.05% | sector strong でも救済しない |
| `neutral_rerating green` | `sector_strong` | 537 | +1.692% | 60.34% | 2.42% | 強いが件数は少ない |

旧v2では `neutral_rerating blue + low_pbr20_low_fwd_per20 + sector_strong_consistent` の20D median は `+1.716%` だった。v4 の公式指数scoreでは `sector_strong` が `+2.055%` まで上がり、ユーザーがチャートで見ているsector betaをより素直に拾う。ただし採用版では、公式指数だけでなく構成銘柄の広がりも半分入れる。
...
- `sector_strength_score` は公式33業種指数 score と構成銘柄 score の平均を使う。公式指数側の price action は `indices_data` / `index_master.category='sector33'`、構成銘柄側は PIT sector constituent basket を使う。
- セクターbasketの forward return と `sectorEx` は同日同セクター構成銘柄の等ウェイト平均で、cost / capacity / turnover は含まない。
...
- Bundle v4 index-score reference: `/Users/shinjiroaso/.local/share/trading25/research/market-behavior/ranking-sector-strength-evidence/20260601_ranking_sector_strength_index_score_v4/`
- Bundle v5 average-score adopted: `/Users/shinjiroaso/.local/share/trading25/research/market-behavior/ranking-sector-strength-evidence/20260601_ranking_sector_strength_average_score_v5/`
...
- `sector_index_strength_score` は公式33業種指数の `sector_index_5d_topix_excess_pct`、`sector_index_20d_topix_excess_pct`、`sector_index_60d_topix_excess_pct` と構成銘柄 `sector_breadth_20d_pct` の within-date rank を `0.20 / 0.45 / 0.25 / 0.10` で合成する。
- `sector_constituent_strength_score` は構成銘柄の `sector_constituent_20d_topix_excess_pct`、`sector_constituent_60d_topix_excess_pct`、`sector_breadth_20d_pct` の within-date rank を等ウェイト平均する。
- `sector_strength_score` は `sector_index_strength_score` と `sector_constituent_strength_score` の平均とし、Daily Ranking では `Trade Score` として表示する。

...
この experiment は `Ranking` の赤色候補をすぐ UI rule 化するためのものではない。独立した evidence bundle として、`crowded_rerating + high valuation / no value`、`distribution_stress + high valuation`、`stale_liquidity + high valuation` が long 回避または short candidate として十分に悪い forward distribution を持つかを検証した。
...
| `distribution_stress_high_valuation` | 60D | 75,783 | -1.578% | -4.978% | 60.33% | 38.83% | 24.09% | 60Dでも最も赤に近い |
| `stale_high_valuation` | 60D | 51,675 | -3.045% | -3.616% | 66.14% | 26.38% | 10.47% | 逆行tailが相対的に低く、long 回避/short候補として良い |
| `stale_high_valuation_weak_trend` | 60D | 27,148 | -2.701% | -3.174% | 65.32% | 24.55% | 9.98% | 持続的に弱いが主条件より少し弱い |
...
| `20D > 0 AND 60D > 0` | 25,521 | -1.360% | -1.650% | 61.87% | 5.34% | 24,492 | -3.430% | -4.169% | 67.07% | 11.02% | 最も short red に近い。上昇済み stale high valuation |
...
| `crowded_high_valuation` | all | 43,036 | -1.098% | 54.01% | 21.05% | 19.64% | baseline |
| `crowded_high_valuation` | `atr20_to_atr60_overheat` | 8,536 | -2.060% | 56.90% | 24.18% | 18.86% | crowded high valuation の明確な悪化条件 |
| `distribution_stress_high_valuation` | all | 80,319 | -1.624% | 56.88% | 18.59% | 15.56% | baseline |
| `distribution_stress_high_valuation` | `recent_20d_60d_negative` | 41,596 | -1.800% | 58.00% | 17.62% | 13.97% | trend weak で少し悪化 |
| `distribution_stress_high_valuation` | `atr20_to_atr60_overheat` | 4,847 | -0.563% | 52.16% | 15.95% | 18.71% | overheat では逆行tailが増え、単純悪化にならない |
| `stale_high_valuation_weak_trend` | all | 28,659 | -1.309% | 60.36% | 5.79% | 4.09% | baseline |
| `stale_high_valuation_weak_trend` | `atr20_acceleration` | 2,775 | -1.951% | 66.02% | 6.77% | 4.86% | stale では ATR加速が悪化寄り |
...
| --- | --- |
| 1 | `stale_high_valuation` | short red の base condition |
| 2 | `stale_high_valuation + 20D>0 AND 60D>0` | base red 内の強化状態。`stale_rally_fade` / `stale_high_valuation_recent_positive` として扱う |
| 3 | `distribution_stress_high_valuation` | red-risk / caution。逆行上方tailが大きいため base red にはしない |

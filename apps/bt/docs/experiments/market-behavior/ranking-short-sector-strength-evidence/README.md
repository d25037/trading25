...
例として `2914` 日本たばこ産業は 2026-05-28 時点で `stale_liquidity`、PBR percentile `0.819`、20D return `+4.61%`、60D return `+1.46%` のため、現行 Daily Ranking では赤 `Stale` + `Rally Fade` に該当する。一方、Balanced Sector Strength は `0.33` の `sector_neutral` であり、これは `sector_weak` 型ではなく `stale_rally_fade` 型の relative red / long回避候補として読む。
...
- Balanced Sector Strength は TOPIX-relative の PIT state なので、raw return と excess return を必ず併記する。

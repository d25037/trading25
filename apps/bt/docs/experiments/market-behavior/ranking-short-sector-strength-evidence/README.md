...
short 側の sector overlay は「long の逆で常に sector weak が良い」とは扱わない。`crowded_rerating` / `distribution_stress` の高valuation・no value 候補では `sector_weak` が明確な short tailwind になる。一方、`stale_high_valuation` は sector weak よりも「stale そのもの」と「recent positive rally fade」の効果が強く、20D では `sector_strong` でも TOPIX excess は悪い。したがって production 表示では、short 候補の主条件を value / liquidity / technical で決め、sector score は候補ごとに downgrade / confirmation として読む。
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| `crowded_high_valuation` | `sector_weak` | 3,364 | -1.623% | 56.81% | 22.27% | -1.487% | 最も純shortに近い |
| `crowded_high_valuation` | `sector_strong` | 6,577 | -0.910% | 53.03% | 22.43% | +0.846% | relative short 寄り |
...
したがって、売りbucketは `crowded_high_valuation/no_value + sector_weak` を優先し、`sector_strong` は裸shortより relative short / long回避として扱う。`strong_low_value + sector_strong` は明確に short-prohibit。
...
| `crowded_high_valuation` | weak | 4,406 | -2.070% | +0.732% | -2.801% | -3.638% | 64.68% | 26.60% | 13.14% | high valuation crowded は sector weak で赤に近い |
| `crowded_high_valuation` | neutral | 30,925 | +2.424% | +1.410% | +1.014% | -0.569% | 52.06% | 19.36% | 19.88% | sector weak なしでは mean が残る |
| `distribution_stress_high_valuation` | weak | 17,774 | -0.390% | +1.216% | -1.605% | -2.321% | 60.30% | 19.90% | 12.24% | stress red-risk の一番自然な強化条件 |
| `distribution_stress_high_valuation` | neutral | 56,903 | +1.878% | +1.855% | +0.023% | -1.352% | 55.72% | 17.90% | 16.45% | caution 止まり |
| `distribution_stress_high_valuation` | strong | 5,596 | +1.482% | +2.091% | -0.609% | -2.011% | 58.20% | 21.82% | 16.92% | excess は悪いが raw は強い |
...
| `crowded_high_valuation + overheat` | weak | 781 | -3.421% | -4.437% | 66.45% | 29.96% | 13.96% | crowded short の最有力条件 |
| `crowded_high_valuation + overheat` | neutral | 5,719 | +0.539% | -1.344% | 54.59% | 21.44% | 18.90% | tail はあるが mean は残る |
| `distribution_stress_high_valuation + overheat` | weak | 1,135 | -1.014% | -1.941% | 56.83% | 20.26% | 16.21% | stress では overheat より high valuation + weak sector が主条件 |

#### 結論: stale high valuation は sector weak だけで説明しない

`stale_high_valuation` はもともと TOPIX excess が悪い。sector weak は 60D では効くが、20D では sector strong でも悪く、単純な「弱セクターだけ short」とは言いにくい。むしろ `20D > 0 AND 60D > 0` の stale rally fade が sector を問わず悪い。

| Stale split | Sector | Horizon | Obs | Excess mean | Excess median | Negative excess | Downside excess tail | Upside excess tail |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `all_stale_high_valuation` | weak | 20D | 8,242 | -1.041% | -0.933% | 57.63% | 7.12% | 4.48% |
| `all_stale_high_valuation` | neutral | 20D | 40,677 | -1.197% | -1.469% | 61.31% | 6.89% | 4.52% |
| `all_stale_high_valuation` | strong | 20D | 5,279 | -1.730% | -2.465% | 64.80% | 9.98% | 6.27% |
...
`short_value_sector_interaction_df` では、high valuation / no value は sector weak で悪化し、strong low value は short に向かない。特に `strong_low_value_sector_strong_short_prohibit` は 20D excess mean `+2.037%`、60D excess mean `+5.464%` で、short 色に混ぜるべきではない。

| Priority condition | Horizon | Obs | Raw mean | TOPIX mean | Excess mean | Excess median | Negative excess | Upside excess tail |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `crowded_high_valuation_overheat_sector_weak` | 20D | 781 | -2.877% | +0.544% | -3.421% | -4.437% | 66.45% | 13.96% |
| `crowded_no_value_overheat_sector_weak` | 20D | 937 | -2.630% | +0.559% | -3.188% | -3.953% | 65.10% | 13.13% |
| `distribution_stress_high_valuation_sector_weak` | 20D | 17,774 | -0.390% | +1.216% | -1.605% | -2.321% | 60.30% | 12.24% |
...
short 側で sector weak が最も明確に効くのは、`crowded` の high valuation / no value と、`distribution_stress_high_valuation`。ここでは raw return も弱く、TOPIX が上がる中で個別がさらに負けているため、セクターベータが short thesis を補強している。

`stale_high_valuation` は読み方が違う。stale は「弱セクターだから落ちる」というより、流動性が stale な高valuation銘柄が TOPIX に負けやすい bucket。sector strong でも 20D excess は悪く、sector weak に限定しすぎると stale red の本体を取りこぼす可能性がある。60D では `stale_rally_fade_sector_weak` が最も素直に悪いため、長めの relative short / long回避では sector weak を強い confirmation として扱える。

value confirmation は short 側でも重要。high valuation と no value は short red の方向に寄るが、strong low value は sector strong と重なると明確に short 禁止。現在の short-side 配色を見直す場合も、long-side と同じく no value / high valuation / strong value を分けるべきで、単に liquidity regime だけで赤にしない。
...
| --- | --- | --- |
| 1 | `crowded_high_valuation + ATR overheat` | `sector_weak` で red-risk 昇格候補。`sector_neutral/strong` では caution |
| 2 | `crowded_no_value + ATR overheat` | `sector_weak` で red-risk 昇格候補。value なしの crowded を救済しない |
| 3 | `distribution_stress_high_valuation` | `sector_weak` で short confirmation。strong sector では raw return が残るため弱める |
| 4 | `stale_high_valuation` | sector で hard filter しない。`stale_rally_fade` と 60D sector weak を強化状態として表示 |
| 5 | `strong_low_value + sector_strong` | short 禁止 / downgrade。赤側に混ぜない |

#### 実務上の読み替え

`sector_weak` 単独を狙うのではなく、もともと short/red 候補として悪い個別条件に弱セクターベータが乗るかを見る。`crowded` / `stress` は `sector_weak` の有無で red-risk の強さがかなり変わるため、`crowded_high_valuation + ATR overheat + sector_weak` と `distribution_stress_high_valuation + sector_weak` を優先する。

一方で `stale_high_valuation` は `sector_weak` を前提にしない。`stale_high_valuation` 全体を base red / long回避に置き、`20D > 0 AND 60D > 0` の `stale_rally_fade` を本命の強化状態として扱う。20D では `sector_strong` でも TOPIX excess は悪いため、sector strong を機械的に除外しない。ただし裸shortでは強セクターベータによる逆行に注意し、`sector_weak` は安全寄り confirmation、`sector_strong` は relative short なら許容するが risk caution と読む。

...
Same-day follow-up run: `20260702_sma5_count_long_same_day_prime_2018_v1`
...
同日内比較の follow-up は `analysis_start_date=2018-01-01` に広げ、同じ date / market / scaffold の中で `0/1`, `2/3`, `4/5` の group median return を比較した。spread は `comparison group - base group` で、例えば `base=0/1, comparison=4/5` の spread がプラスなら、同じ日に `4/5` が `0/1` を上回ったという意味。
...
2018年以降の same-day matched comparison では、`Neutral + Deep Value + Long Hybrid + ATR20 Accel` の5Dは `0/1` が悪いとは言い切れない。むしろ `0/1 -> 2/3` / `0/1 -> 4/5` の5D spread はプラスだが matched date は `66-67` と薄く、短期 rebound と mixed timing が混ざる。
...
- Same-day bundle: `~/.local/share/trading25/research/market-behavior/ranking-sma5-count-long-evidence/20260702_sma5_count_long_same_day_prime_2018_v1/`

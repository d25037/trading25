...
- Published run: `20260719_prime_pit_technical_fit_shape_v10`
- Immutable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v10/`
- Provenance commit: `e55781ef9e4b2fef74fb47d145f272cef97071ae` (`git_dirty=false`)
...
PR 480 final review後のv9 rerunでは、N225 sensitivityを銘柄固有のauthoritative completion dateに揃えた。v8の10,000-row sampleと比べ、N225 excessは5D 70 rows（最大`3.136705 pp`）、20D 382 rows（最大`5.113219 pp`）、60D 1,217 rows（最大`5.373153 pp`）変化したため、v8をimmutable archiveとしてsupersedeした。publication reviewでv9 artifactの`invalidation_disposition`がこのv8→v9 endpoint repairを記録していないことが判明したため、v10は同じtable evidenceをclean rerunし、provenance chainへ明示的に追記した。primary TOPIX tables、digest metrics、decision gateは変わらず、最終判断は`neither`のままである。
...
v10の`topk_operational_lift`は12,956 complete rowsと340 `incomplete_outcomes` audit rowsを区別する。未完了outcomeのmetricは`NULL`で、上表とadoption gateにはcomplete rowsだけを使う。
...
published v10 は physical `market.duckdb` schema v4 と `stock_price_adjustment_mode=local_projection_v2_event_time` を要求する。Technical Fit の株価 source は `stock_data_raw` のみで、`stock_data` fallback はない。
...
- invalidation disposition: `v1_v2_historical_archive_v3_superseded_by_v4_for_price_basis_gate_ci_hardening_v4_superseded_by_v5_for_explicit_failed_shape_slices_v5_superseded_by_v6_for_lineage_disposition_hardening_v6_superseded_by_v7_for_review_fixed_frontier_and_flat_mapping_v7_superseded_by_v8_for_lineage_disposition_hardening_v8_superseded_by_v9_for_completion_aligned_n225_endpoint_repair`
...
- v10: v8→v9 N225 endpoint repairをmanifest/summaryのprovenance chainへ明示した現行canonical artifact。v1–v10はすべてimmutableで、旧bundleを変更・削除しない。
...
v10でもnear-flat guardrailの境界を維持し、実データの150 mappingはいずれもthreshold外だった。N225 endpointとそのsupersession lineageを揃えてもprimary TOPIX evidenceとdecisionは不変である。
...
- near-flat thresholdは将来のexpectancy spread `<=0.01 pp` mappingをneutral `0.5`へ束ねるcontractだが、v10 artifactでは該当rowがないため、flat regimeの実証結果ではない。
...
- v10のlineage-complete rerunでもgateは変わらず、API、materialization、UIのfollow-onを開始しない。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v10/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v10/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v10/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v10/summary.md`
- Provenance: manifest git commit `e55781ef9e4b2fef74fb47d145f272cef97071ae`; `git_dirty=false`。artifact SHA-256はmanifest `446ffd75762c7c44802d5c0e2a5122d8409028da1eb043266833a13f214ce2a5`、results `d56a6e3ad650ac40d39d19724f324b7b95a1666ed2adb7f881199ba24545df12`、summary `5801252d010a38a592cb6e92be96e8cab29506924c5796c11e0a9d27c21c9bbb`。v9はmanifest/results/summary SHA-256 `a04de07b…` / `a4a95693…` / `e12ba806…`でimmutable保持する。
...
Validation: exact 15 non-empty tables、433,882 observations、Prime-only coverage `0101,0111`、same-date fixed/OLS pairing、completion dateがsignal dateより後、24 shape-gate rows、150 `ready` / 0 `flat` mappings、Top-K 12,956 complete / 340 incomplete audit rows、旧 `oos_*` flagsなし、price audit counts/hashes、artifact内のv8→v9 supersession reason、summary/manifest/decision gateの `neither` 一致を確認した。
...
uv run --project apps/bt python \
  ...
  --run-id 20260719_prime_pit_technical_fit_shape_v10
```

...
- Published run: `20260719_prime_pit_technical_fit_shape_v9`
- Immutable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v9/`
- Provenance commit: `26abfac09b92d0af7e458873d9f397f010912f0c` (`git_dirty=false`)
...
v9の`topk_operational_lift`は12,956 complete rowsと340 `incomplete_outcomes` audit rowsを区別する。未完了outcomeのmetricは`NULL`で、上表とadoption gateにはcomplete rowsだけを使う。
...
published v9 は physical `market.duckdb` schema v4 と `stock_price_adjustment_mode=local_projection_v2_event_time` を要求する。Technical Fit の株価 source は `stock_data_raw` のみで、`stock_data` fallback はない。
...
- v9: authoritative stock completion dateにN225 endpointを揃えた現行canonical artifact。v1–v9はすべてimmutableで、旧bundleを変更・削除しない。
...
v9でもnear-flat guardrailの境界を維持し、実データの150 mappingはいずれもthreshold外だった。N225 endpointを揃えてもprimary TOPIX evidenceとdecisionは不変である。
...
- near-flat thresholdは将来のexpectancy spread `<=0.01 pp` mappingをneutral `0.5`へ束ねるcontractだが、v9 artifactでは該当rowがないため、flat regimeの実証結果ではない。
...
- v9のcompletion-aligned rerunでもgateは変わらず、API、materialization、UIのfollow-onを開始しない。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v9/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v9/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v9/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v9/summary.md`
- Provenance: manifest git commit `26abfac09b92d0af7e458873d9f397f010912f0c`; `git_dirty=false`。artifact SHA-256はmanifest `a04de07b9ef1ba5d2ba94531035aea83de07e009f714d77386a887b783c5c291`、results `a4a956931bb984b7b93b41f65396a072e50ca51708b90325e1d9a7940662c0eb`、summary `e12ba806409e4791737f5f73d6315ca4da16fda88355b9067a80f449a722b315`。v8はmanifest/results/summary SHA-256 `99983c57…` / `08162cb0…` / `e12ba806…`でimmutable保持する。
...
uv run --project apps/bt python \
  ...
  --run-id 20260719_prime_pit_technical_fit_shape_v9
```

...
- Published run: `20260719_prime_pit_technical_fit_shape_v11`
- Immutable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v11/`
- Provenance commit: `662d7ca5c99df741bc596517514be11de64a3d6a` (`git_dirty=false`)
...
PR 480 final review後のv9 rerunでは、N225 sensitivityを銘柄固有のauthoritative completion dateに揃えた。v8の10,000-row sampleと比べ、N225 excessは5D 70 rows（最大`3.136705 pp`）、20D 382 rows（最大`5.113219 pp`）、60D 1,217 rows（最大`5.373153 pp`）変化したため、v8をimmutable archiveとしてsupersedeした。v10は欠落していたv8→v9 transitionを追記したが、自身をcanonicalにするv9→v10 transitionを持たず、chainが一世代遅れのままだった。v11は同じtable evidenceをclean rerunし、`v9_superseded_by_v10_for_missing_v8_v9_lineage`と`v10_superseded_by_v11_for_missing_v9_v10_lineage`を同時に事前記録してcanonical chainをv11まで閉じた。primary TOPIX tables、digest metrics、decision gateは変わらず、最終判断は`neither`のままである。
...
v11の`topk_operational_lift`は12,956 complete rowsと340 `incomplete_outcomes` audit rowsを区別する。未完了outcomeのmetricは`NULL`で、上表とadoption gateにはcomplete rowsだけを使う。
...
published v11 は physical `market.duckdb` schema v4 と `stock_price_adjustment_mode=local_projection_v2_event_time` を要求する。Technical Fit の株価 source は `stock_data_raw` のみで、`stock_data` fallback はない。
...
- v11: v9→v10の欠落理由とv10→v11のcanonical transitionを同時に事前記録し、provenance chainをv11まで閉じた現行canonical artifact。v1–v11はすべてimmutableで、旧bundleを変更・削除しない。
...
v11でもnear-flat guardrailの境界を維持し、実データの150 mappingはいずれもthreshold外だった。N225 endpointとcanonical publicationまで閉じたsupersession lineageを揃えてもprimary TOPIX evidenceとdecisionは不変である。
...
- near-flat thresholdは将来のexpectancy spread `<=0.01 pp` mappingをneutral `0.5`へ束ねるcontractだが、v11 artifactでは該当rowがないため、flat regimeの実証結果ではない。
...
- v11のlineage-closed rerunでもgateは変わらず、API、materialization、UIのfollow-onを開始しない。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v11/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v11/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v11/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v11/summary.md`
- Provenance: manifest git commit `662d7ca5c99df741bc596517514be11de64a3d6a`; `git_dirty=false`。artifact SHA-256はmanifest `3846a70ca3b295913974c78536658abe3fc64da27cc045b8d1e8019c65d5f41d`、results `b9502e638c0a01169c1e5ab3c9f587ede5e0ccd1d1c1d9c83d3f4620ae8cfa87`、summary `a530b87e79de00429f6ed3d7dff78df4107b48f4e61330ab21d021beaf24b526`。v10はmanifest/results/summary SHA-256 `446ffd75…` / `d56a6e3a…` / `5801252d…`、v9は`a04de07b…` / `a4a95693…` / `e12ba806…`でimmutable保持する。
...
Validation: exact 15 non-empty tables、433,882 observations、Prime-only coverage `0101,0111`、same-date fixed/OLS pairing、completion dateがsignal dateより後、24 shape-gate rows、150 `ready` / 0 `flat` mappings、Top-K 12,956 complete / 340 incomplete audit rows、旧 `oos_*` flagsなし、price audit counts/hashes、artifact内のv9→v10/v10→v11 closure、summary/manifest/decision gateの `neither` 一致を確認した。
...
uv run --project apps/bt python \
  ...
  --run-id 20260719_prime_pit_technical_fit_shape_v11
```

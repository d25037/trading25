...
- Published run: `20260719_prime_pit_technical_fit_shape_v7`
- Immutable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v7/`
- Provenance commit: `764de65122af90071cbda266668ae03667342a01` (`git_dirty=false`)
...
PR 480 review後のv7 rerunでは、event-time adjustment frontier、Top-K outcome coverage、raw expectancy spread `<=0.01 pp` のnear-flat mapping contractを固定した。150 walk-forward mappingsはすべて`ready`で、near-flat判定に該当する実データrowは0件だった。published metricsと`neither`判断はv6から変わらない。
...
v7の`topk_operational_lift`は12,956 complete rowsと340 `incomplete_outcomes` audit rowsを区別する。未完了outcomeのmetricは`NULL`で、上表とadoption gateにはcomplete rowsだけを使う。
...
published v7 は physical `market.duckdb` schema v4 と `stock_price_adjustment_mode=local_projection_v2_event_time` を要求する。Technical Fit の株価 source は `stock_data_raw` のみで、`stock_data` fallback はない。
...
- v7: event-time adjustment frontier、明示的なTop-K outcome coverage、`0.01 pp` near-flat thresholdを同一clean commitで再実行した現行canonical artifact。v1–v7 はすべて immutable で、旧bundleを変更・削除しない。
...
v7ではnear-flat guardrailの境界を明示したが、実データの150 mappingはいずれもthreshold外だった。判断不変はguardrailを使わなかったことによる偶然の上書きではなく、review-fixed contractで同じevidenceを再確認した結果である。
...
- near-flat thresholdは将来のexpectancy spread `<=0.01 pp` mappingをneutral `0.5`へ束ねるcontractだが、v7 artifactでは該当rowがないため、flat regimeの実証結果ではない。
...
- v7のreview-fixed rerunでもgateは変わらず、API、materialization、UIのfollow-onを開始しない。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v7/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v7/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v7/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-technical-fit-score-shape-evidence/20260719_prime_pit_technical_fit_shape_v7/summary.md`
...
- Run telemetry: wall `528.98s`; maximum RSS `9,933,357,056 bytes`; swap `0`。
...
uv run --directory apps/bt python \
  ...
  --run-id 20260719_prime_pit_technical_fit_shape_v7 \
  --notes 'PR 480 review-fixed rerun at 764de651: event-time adjustment frontier enforced; near-flat raw expectancy mappings bounded at 0.01 pp; frozen v6 study gates and parameters retained.'
```

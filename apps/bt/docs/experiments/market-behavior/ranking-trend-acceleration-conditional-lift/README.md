...
PR 480 review後のv5 rerunでは、overlap membershipを保持して独立slice判定と分離し、event-time adjustment frontierおよびTop-K outcome coverageを明示監査した。continuous `1 / 7`、binary `0 / 7`、`reject_introduction`という最終判断は変わらない。
...
v5の`topk_priority_lift_df`は23,653 complete rowsと221 `incomplete_outcomes` rowsを区別する。未完了outcomeを暗黙に成績0へ変換しないreview-fixed集計でも、published lift、CI、gate verdictは上記のままである。
...
v5はreview後のcanonical evidenceであり、旧v4の数値だけを根拠にfollow-onを再開しない。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v5/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v5/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v5/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v5/summary.md`
- Bundle provenance: manifest `git_commit` は `764de65122af90071cbda266668ae03667342a01`、`git_dirty` は `false`。overlap membership、event-time adjustment frontier、Top-K outcome coverage、complete price auditを同 commitで固定した。
...
- Run telemetry: wall `266.45s`; maximum RSS `10,228,383,744 bytes`; swap `0`
- Runner command:
...
uv run --directory apps/bt python \
  ...
  --run-id 20260719_prime_price_pit_conditional_lift_v5 \
  --notes 'PR 480 review-fixed rerun at 764de651: overlapping candidate membership preserved; event-time adjustment frontier enforced; frozen v4 study gates and parameters retained.'
```

...
PR 480 final review後のv6 rerunでは、銘柄固有の実completion dateをstock / TOPIX / N225の全endpointに透過し、旧v5のnominal benchmark leadを廃止した。overlap membership、event-time adjustment frontier、Top-K outcome coverageとfrozen gateは維持している。continuous `1 / 7`、binary `0 / 7`、`reject_introduction`という最終判断は変わらない。
...
v6の`topk_priority_lift_df`は23,653 complete rowsと221 `incomplete_outcomes` rowsを区別する。未完了outcomeを暗黙に成績0へ変換しないaligned-outcome集計でも、published lift、CI、gate verdictは上記のままである。
...
v6はaligned-outcome canonical evidenceであり、旧v5の数値だけを根拠にfollow-onを再開しない。
...
Endpoint alignment auditではPrime equivalent全complete keyのstock completion / nominal TOPIX completion不一致が5D `1,044 / 4,502,049`、20D `2,509 / 4,473,964`、60D `4,431 / 4,399,245`あった。v5の10,000-row sampleはcode `7916`の60Dで4 signal datesが不一致となり、TOPIX-excessの絶対差は最大`1.102532 pp`だった。v6の`observation_sample_df`は5D/20D/60Dそれぞれのcompletion date、stock return、TOPIX excessを保持し、N225も同じcompletion dateで結合する。
...
- Durable bundle: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v6/`
- Manifest: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v6/manifest.json`
- Results: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v6/results.duckdb`
- Summary: `~/.local/share/trading25/research/market-behavior/ranking-trend-acceleration-conditional-lift/20260719_prime_price_pit_conditional_lift_v6/summary.md`
- Bundle provenance: manifest `git_commit` は `26abfac09b92d0af7e458873d9f397f010912f0c`、`git_dirty` は `false`。artifact SHA-256はmanifest `967925a573486ecb7b578c86caa0cf20de937665dd048818aa1eee9202b83bea`、results `b98ec3e3ae9cbf403bc1bad9322c789a808c628ae0ca0e42a8d9e7495b91d844`、summary `8e214ad36a4028bde5a927e8e3a02494496acb707d8e5626c00f5b69007405cd`。

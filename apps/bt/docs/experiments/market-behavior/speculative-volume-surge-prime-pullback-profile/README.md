...
- Blocker: Prime label must be PIT-resolved.
- `Prime` として読む profile は signal-date `stock_master_daily` で再実行するまで proxy context に留める。
...
### Source Artifacts
- Experiment: `market-behavior/speculative-volume-surge-prime-pullback-profile`

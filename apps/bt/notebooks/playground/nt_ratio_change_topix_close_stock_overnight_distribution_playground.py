...
def _(get_topix_close_return_stats, parsed_inputs):
    try:
        _sigma_threshold_1, _sigma_threshold_2 = parsed_inputs[
            "topix_sigma_thresholds"
        ]
        ...
        topix_stats_error_message = None
    except Exception as exc:
        topix_close_stats = None
        topix_stats_error_message = str(exc)
    return topix_close_stats, topix_stats_error_message
...
def _(
    ...
    run_nt_ratio_change_topix_close_stock_overnight_distribution,
    topix_close_stats,
    topix_stats_error_message,
):
    try:
        _sigma_threshold_1, _sigma_threshold_2 = parsed_inputs["sigma_thresholds"]
        if topix_stats_error_message:
            raise ValueError(topix_stats_error_message)
        if topix_close_stats is None:
...
@app.cell
def _(error_message, mo, parsed_inputs, result, topix_close_stats):
    _summary_view = mo.md("")
    if not error_message and result is not None:
        ...
        _topix_stats = topix_close_stats
        _nt_stats_lines = [
        ...
        if _nt_stats is not None:
            _nt_stats_lines = [
                ...
                f"- NT bucket boundaries: **μ-{_sigma_threshold_2:g}σ = {_nt_stats.lower_threshold_2 * 100:.4f}%**, **μ-{_sigma_threshold_1:g}σ = {_nt_stats.lower_threshold_1 * 100:.4f}%**, **μ+{_sigma_threshold_1:g}σ = {_nt_stats.upper_threshold_1 * 100:.4f}%**, **μ+{_sigma_threshold_2:g}σ = {_nt_stats.upper_threshold_2 * 100:.4f}%**",
        ...
        if _topix_stats is not None:
            _topix_stats_lines = [
                ...
                f"- TOPIX sigma thresholds: **{_topix_sigma_threshold_1:g}σ / {_topix_sigma_threshold_2:g}σ**",
        ...
        _summary_view = mo.md(
            "\n".join(
                [
                    ...
                    f"- NT sigma thresholds: **{_sigma_threshold_1:g}σ / {_sigma_threshold_2:g}σ**",
                    f"- Sample size per group/joint bucket: **{parsed_inputs['selected_sample_size']}**",
...
@app.cell
def _(error_message, mo, pd, result, topix_close_stats):
    _stats_table = mo.md("")
    if not error_message and result is not None:
        _views = []
        ...
        if topix_close_stats is not None:
            _topix_stats_df = pd.DataFrame(
                [
                    {
                        "sample_count": topix_close_stats.sample_count,
                        "mean_return": topix_close_stats.mean_return,
                        "std_return": topix_close_stats.std_return,
                        "threshold_1": topix_close_stats.threshold_1,
                        "threshold_2": topix_close_stats.threshold_2,
                        "min_return": topix_close_stats.min_return,
                        "q25_return": topix_close_stats.q25_return,
                        "median_return": topix_close_stats.median_return,
                        "q75_return": topix_close_stats.q75_return,
                        "max_return": topix_close_stats.max_return,
                    }
                ]
            )
            _views.extend(

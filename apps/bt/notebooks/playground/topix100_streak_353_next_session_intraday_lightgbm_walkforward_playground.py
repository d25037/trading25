...
app = marimo.App(
    width="full",
    app_title="TOPIX100 Streak 3/53 Next-Session Intraday LightGBM Walk-Forward",
)
...
def _(Path, sys):
    ...
    from src.domains.analytics.topix100_streak_353_next_session_intraday_lightgbm_walkforward import (
        get_topix100_streak_353_next_session_intraday_lightgbm_walkforward_bundle_path_for_run_id,
        get_topix100_streak_353_next_session_intraday_lightgbm_walkforward_latest_bundle_path,
        load_topix100_streak_353_next_session_intraday_lightgbm_walkforward_research_bundle,
    )
...
@app.cell
def _(get_latest_bundle_defaults, get_topix100_streak_353_next_session_intraday_lightgbm_walkforward_latest_bundle_path):
...
def _(build_bundle_viewer_controls, latest_bundle_path_str, latest_run_id, mo):
    run_id, bundle_path, controls_view = build_bundle_viewer_controls(
        ...
        runner_path="apps/bt/scripts/research/run_topix100_streak_353_next_session_intraday_lightgbm_walkforward.py",
        extra_note_lines=[
            "- `walkforward_model_comparison_df` is the main OOS read",
            "- `portfolio_stats_df` is the execution read for long / short / spread / pair 50/50",
            "- `daily_return_distribution_df` is the percentile view of daily outcomes",
...
def _(
    ...
    try:
        bundle_info, result = load_bundle_selection(
            ...
            load_research_bundle=load_topix100_streak_353_next_session_intraday_lightgbm_walkforward_research_bundle,
        )
        error_message = None
    ...
    if error_message or result is None:
        ...
        content = mo.vstack(
            [
                ...
                mo.md("## Execution Portfolio Stats"),
                mo.Html(result.portfolio_stats_df.round(4).to_html(index=False)),
                mo.md("## Daily Return Distribution"),
                mo.Html(result.daily_return_distribution_df.round(4).to_html(index=False)),

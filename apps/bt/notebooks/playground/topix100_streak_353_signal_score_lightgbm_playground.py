...
app = marimo.App(
    width="full",
    app_title="TOPIX100 Streak 3/53 Signal Score LightGBM",
)
...
@app.cell
def _(get_latest_bundle_defaults, get_topix100_streak_353_signal_score_lightgbm_latest_bundle_path):
...
def _(build_bundle_viewer_controls, latest_bundle_path_str, latest_run_id, mo):
    run_id, bundle_path, controls_view = build_bundle_viewer_controls(
        ...
        runner_path="apps/bt/scripts/research/run_topix100_streak_353_signal_score_lightgbm.py",
        extra_note_lines=[
            "- `validation_model_comparison_df` is the main baseline-vs-LightGBM read",
            "- `validation_model_summary_df` shows the actual top-k edge by side",
            "- `feature_importance_df` is the quickest way to see whether continuous features matter",
        ],
    )
    controls_view
...
def _(
    ...
    try:
        bundle_info, result = load_bundle_selection(
            ...
            load_research_bundle=load_topix100_streak_353_signal_score_lightgbm_research_bundle,
        )
        error_message = None

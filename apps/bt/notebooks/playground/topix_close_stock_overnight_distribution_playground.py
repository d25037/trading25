...
def _(STOCK_GROUP_ORDER, default_db_path, initial_range, mo):
    ...
    close_threshold_1 = mo.ui.number(
        value=1.0,
        start=0.1,
        stop=10.0,
        step=0.1,
        label="TOPIX Close Threshold 1 (%)",
    )
    close_threshold_2 = mo.ui.number(
        value=2.0,
        start=0.2,
        stop=20.0,
        step=0.1,
        label="TOPIX Close Threshold 2 (%)",
    )
    ...
    mo.vstack(
        [
            ...
            selected_groups,
            mo.hstack([close_threshold_1, close_threshold_2]),
...
def _(
    ...
    parsed_inputs = {
        ...
        "thresholds": (
            float(close_threshold_1.value) / 100.0,
            float(close_threshold_2.value) / 100.0,
        ),
    }
...
@app.cell
def _(parsed_inputs, run_topix_close_stock_overnight_distribution):
    try:
        ...
        result = run_topix_close_stock_overnight_distribution(
            ...
            close_threshold_1=_threshold_1,
            close_threshold_2=_threshold_2,
...
def _(error_message, mo, parsed_inputs, result):
    ...
    - Excluded TOPIX days without next session: **{result.excluded_topix_days_without_next_session}**

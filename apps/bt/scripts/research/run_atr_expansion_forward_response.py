#!/usr/bin/env python3
"""Runner-first entrypoint for ATR expansion forward-response research."""
...
from src.domains.analytics.atr_expansion_forward_response import (  # noqa: E402
    DEFAULT_ATR_WINDOWS,
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe ATR20/ATR60 expansion research over daily stock anchors "
            "and forward 5d/20d TOPIX-excess returns."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Comma-separated ATR windows in trading sessions. Currently expects 20,60.",
    )
    parser.add_argument(
        "--return-windows",
        default=",".join(str(window) for window in DEFAULT_RETURN_WINDOWS),
        help="Comma-separated recent-return windows in trading sessions.",
    )
    parser.add_argument(
        "--horizons",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_atr_expansion_forward_response_research(
        args.db_path,
        start_date=args.start_date,
        end_date=args.end_date,
        atr_windows=_parse_positive_ints(args.atr_windows, name="atr-windows"),
        return_windows=_parse_positive_ints(
            args.return_windows,
            name="return-windows",
        ),

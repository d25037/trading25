#!/usr/bin/env python3
"""Runner-first entrypoint for forward EPS trade-archetype decomposition."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Dataset name to analyze. Defaults to {DEFAULT_DATASET_NAME}.",
    )
    parser.add_argument(
        "--holdout-months",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_forward_eps_trade_archetype_decomposition(
    ...
    bundle = write_forward_eps_trade_archetype_decomposition_bundle(
        result,

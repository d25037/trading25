#!/usr/bin/env python3
"""Runner-first entrypoint for NT ratio change x TOPIX close overnight research."""
...
from src.domains.analytics.nt_ratio_change_topix_close_stock_overnight_distribution import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run NT ratio change x TOPIX close / stock overnight distribution "
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="NT ratio lower sigma threshold.",
    )
    parser.add_argument(
        "--sigma-threshold-2",
        type=float,
        default=2.0,
        help="NT ratio upper sigma threshold.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="Deterministic sample size per group/joint bucket.",
    )
    parser.add_argument(
        "--clip-lower",
        type=float,
...
def main(argv: list[str] | None = None) -> int:
    ...
    topix_close_stats = get_topix_close_return_stats(
        ...
        sigma_threshold_1=args.topix_sigma_threshold_1,
        sigma_threshold_2=args.topix_sigma_threshold_2,
    )

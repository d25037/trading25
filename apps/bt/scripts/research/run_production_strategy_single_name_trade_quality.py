#!/usr/bin/env python3
"""Runner-first entrypoint for single-name trade-quality audit."""
...
from src.domains.analytics.production_strategy_single_name_trade_quality import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a reproducible single-name trade-quality audit for production "
            "strategies without integrated portfolio sizing."
        )
    )
    parser.add_argument(
        "--strategy",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_production_strategy_single_name_trade_quality(
    ...
    bundle = write_production_strategy_single_name_trade_quality_bundle(
        result,

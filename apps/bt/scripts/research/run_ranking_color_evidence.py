#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking valuation/liquidity color evidence."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run fast PIT-safe Ranking color evidence research using daily_valuation "
            "market-percentile buckets and forward TOPIX-excess returns."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_color_evidence_research(
        args.db_path,

#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking core factor-regime breakdown research."""
...
from src.domains.analytics.ranking_core_factor_regime_breakdown import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Break down why the momentum-value Ranking core changes by year, "
            "especially the 2026 partial factor regime."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Comma-separated forward return horizons. Must include 5,10,20,60.",
    )
    parser.add_argument(
        "--markets",
    ...
    parser.add_argument(
        ...
        help="Minimum finite names per date x market x sector valuation group.",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_core_factor_regime_breakdown_research(
        args.db_path,

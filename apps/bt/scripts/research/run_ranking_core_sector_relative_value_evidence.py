#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking core sector-relative value evidence."""
...
from src.domains.analytics.ranking_core_sector_relative_value_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare raw and 33-sector-relative valuation ranks inside the "
            "neutral-rerating sector-strong core sleeve."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_core_sector_relative_value_evidence_research(
        args.db_path,

#!/usr/bin/env python3
"""Runner-first entrypoint for Crowded long left-tail evidence."""
...
from src.domains.analytics.ranking_crowded_long_tail_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
    DEFAULT_MARKET_SCOPES,
    DEFAULT_MIN_OBSERVATIONS,
    DEFAULT_LONG_HYBRID_THRESHOLD,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe Daily Ranking evidence for Crowded Rerating + "
            "Long Hybrid >= 0.8 left-tail decomposition."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Anchor start date. Defaults to 2023-01-01 for fast PSR-era research.",
    )
    ...
    parser.add_argument(
        "--long-hybrid-threshold",
        type=float,
        default=DEFAULT_LONG_HYBRID_THRESHOLD,
        help="Long Hybrid Leadership threshold for the crowded long scaffold.",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_crowded_long_tail_evidence_research(
        args.db_path,
        ...
        long_hybrid_threshold=args.long_hybrid_threshold,

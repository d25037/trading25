#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking forecast operating-profit growth evidence."""
...
from src.domains.analytics.ranking_forecast_operating_profit_growth_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe Daily Ranking evidence for forecast operating-profit "
            "growth ratio and PER/Fwd PER to growth ratio parameters."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_forecast_operating_profit_growth_evidence_research(
        args.db_path,

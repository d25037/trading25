#!/usr/bin/env python3
"""Runner-first entrypoint for liquidity x price-action recomposition research."""
...
from src.domains.analytics.ranking_liquidity_price_action_recomposition import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking evidence that decomposes high-liquidity "
            "rerating/stress into 20D x 60D price-action buckets."
        )
    )
    parser.add_argument(
        "--db-path",

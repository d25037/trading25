#!/usr/bin/env python3
"""Runner-first entrypoint for liquidity-z long-side evidence research."""
...
from src.domains.analytics.ranking_liquidity_z_long_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking long-side evidence for liquidity_residual_z "
            "buckets and upper-cap sweeps inside rerating price action."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_liquidity_z_long_evidence_research(
        args.db_path,

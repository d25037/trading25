#!/usr/bin/env python3
"""Runner-first entrypoint for long-scaffold factor-cross evidence."""
...
from src.domains.analytics.ranking_long_scaffold_factor_cross_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking long-scaffold evidence crossed with z 0..2 "
            "rerating, Fwd OP/OP > 1.2, and Good Fwd PER."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_long_scaffold_factor_cross_evidence_research(
        args.db_path,

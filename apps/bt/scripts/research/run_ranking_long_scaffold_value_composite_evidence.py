#!/usr/bin/env python3
"""Runner-first entrypoint for long-scaffold value-composite evidence."""
...
from src.domains.analytics.ranking_long_scaffold_value_composite_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking long-scaffold evidence for an equal-weight PIT "
            "value composite of low forward PER percentile and low PBR percentile."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_long_scaffold_value_composite_evidence_research(
        args.db_path,

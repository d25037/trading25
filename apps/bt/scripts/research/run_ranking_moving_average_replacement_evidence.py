#!/usr/bin/env python3
"""Runner-first entrypoint for Daily Ranking moving-average replacement evidence."""
...
from src.domains.analytics.ranking_moving_average_replacement_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare Daily Ranking fixed 20D/60D return states with "
            "price-vs-SMA20/SMA60 and EMA20/EMA60 replacement states."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_moving_average_replacement_evidence_research(
        args.db_path,

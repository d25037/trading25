#!/usr/bin/env python3
"""Runner-first entrypoint for N225-crowded-rerating Daily Ranking research."""
...
from src.domains.analytics.ranking_n225_crowded_rerating_benchmark import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking crowded-rerating evidence with Nikkei 225 "
            "excess return as the primary benchmark outcome."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        "--liquidity-regimes",
        default=",".join(DEFAULT_LIQUIDITY_REGIMES),
        help="Comma-separated liquidity regimes to include. Defaults to crowded_rerating.",
    )
    parser.add_argument(
        "--min-observations",
        type=int,
        default=DEFAULT_MIN_OBSERVATIONS_N225,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_n225_crowded_rerating_benchmark_research(
        ...
        liquidity_regimes=_parse_strings(args.liquidity_regimes, name="liquidity_regimes"),

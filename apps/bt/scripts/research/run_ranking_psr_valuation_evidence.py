#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking PSR valuation evidence."""
...
from src.domains.analytics.ranking_psr_valuation_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe Daily Ranking evidence for actual FY PSR as a "
            "valuation substitute for long/short decision scopes."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_psr_valuation_evidence_research(
        args.db_path,

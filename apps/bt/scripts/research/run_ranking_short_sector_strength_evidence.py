#!/usr/bin/env python3
"""Runner-first entrypoint for Ranking short sector-strength evidence."""
...
from src.domains.analytics.ranking_short_sector_strength_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe Daily Ranking short/red sector-strength evidence over "
            "short candidates, valuation state, stale rally splits, and ATR state."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated market scopes to include: prime, standard, growth, "
            "unknown, or all."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of sample rows to persist.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_short_sector_strength_evidence_research(
        args.db_path,

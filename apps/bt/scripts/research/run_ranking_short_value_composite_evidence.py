#!/usr/bin/env python3
"""Runner-first entrypoint for short-side high fwd PER/PBR composite evidence."""
...
from src.domains.analytics.ranking_short_value_composite_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run short-side Daily Ranking evidence comparing high fwd PER/PBR "
            "composite against PSR-inclusive overvaluation conditions."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Absolute excess-return threshold used for short-side tail diagnostics.",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_short_value_composite_evidence_research(
        args.db_path,

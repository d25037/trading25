#!/usr/bin/env python3
"""Runner-first entrypoint for Daily Ranking SMA5 ATR deviation evidence."""
...
from src.domains.analytics.ranking_sma5_atr_deviation_evidence import (  # noqa: E402
    DEFAULT_ATR_WINDOWS,
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking evidence for close deviation from SMA5 normalized "
            "by ATR5/ATR20, split into above/below threshold diagnostics."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        "--atr-windows",
        default=",".join(str(window) for window in DEFAULT_ATR_WINDOWS),
        help="Comma-separated ATR windows used to normalize SMA5 distance.",
    )
    parser.add_argument(
        "--threshold-abs-atr",
        default=",".join(str(value) for value in DEFAULT_THRESHOLD_ABS_ATR),
        help="Comma-separated absolute ATR-unit thresholds to test by direction.",
    )
    parser.add_argument(
        "--markets",
    ...
    parser.add_argument(
        "--liquidity-bands",
        default=",".join(DEFAULT_LIQUIDITY_BANDS),
        help="Comma-separated liquidity bands for short overlays: high, mid, low.",
    )
    parser.add_argument(
        "--min-observations",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_sma5_atr_deviation_evidence_research(
        ...
        atr_windows=_parse_positive_ints(args.atr_windows, name="atr-windows"),
        threshold_abs_atr=_parse_positive_floats(
            args.threshold_abs_atr,
            name="threshold-abs-atr",
        ),
        market_scopes=_parse_strings(args.markets, name="markets"),
        liquidity_bands=_parse_strings(args.liquidity_bands, name="liquidity-bands"),
...
def _parse_positive_floats(value: str, *, name: str) -> tuple[float, ...]:

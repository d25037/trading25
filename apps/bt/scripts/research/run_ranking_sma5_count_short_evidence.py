#!/usr/bin/env python3
"""Runner-first entrypoint for SMA5 count short-side evidence research."""
...
from src.domains.analytics.ranking_sma5_count_short_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Daily Ranking short-side evidence for the count of days, "
            "within the latest 5 sessions, where close is above SMA5."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Comma-separated liquidity bands to include: high, mid, low.",
    )
    parser.add_argument(
        "--min-observations",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_sma5_count_short_evidence_research(
        args.db_path,
        start_date=args.start_date,
        end_date=args.end_date,
        horizons=_parse_positive_ints(args.horizons, name="horizons"),
        market_scopes=_parse_strings(args.markets, name="markets"),
        liquidity_bands=_parse_strings(args.liquidity_bands, name="liquidity_bands"),

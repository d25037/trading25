#!/usr/bin/env python3
"""Runner-first entrypoint for Prime-only Ranking trend acceleration research."""
...
from src.domains.analytics.ranking_trend_acceleration_conditional_lift import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the Prime-only PIT conditional-lift study for Daily Ranking "
            "trend acceleration inside existing long candidates."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Fixed seed for moving-block bootstrap reproducibility.",
    )
    parser.add_argument(

#!/usr/bin/env python3
"""Runner-first entrypoint for Daily Ranking trend-slope evidence."""
...
from src.domains.analytics.ranking_trend_slope_evidence import (  # noqa: E402
    DEFAULT_HORIZONS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare Daily Ranking fixed 20D/60D return states with rolling "
            "OLS close slope and SMA/EMA slope diagnostic states."
        )
    )
    parser.add_argument(
        "--db-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_ranking_trend_slope_evidence_research(
        args.db_path,

#!/usr/bin/env python3
"""Runner-first entrypoint for recent-return threshold forward-response research."""
...
from src.domains.analytics.recent_return_threshold_forward_response import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PIT-safe general recent-return threshold research over daily stock "
            "anchors and forward 5d/20d TOPIX-excess returns."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        "--thresholds-20d",
        default=",".join(str(value) for value in DEFAULT_20D_THRESHOLDS),
        help="Comma-separated absolute 20d return thresholds in percent.",
    )
    parser.add_argument(
        "--thresholds-60d",
        default=",".join(str(value) for value in DEFAULT_60D_THRESHOLDS),
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_recent_return_threshold_forward_response_research(
        args.db_path,

#!/usr/bin/env python3
"""Runner-first entrypoint for prime-only speculative pullback-profile research."""
...
from src.domains.analytics.speculative_volume_surge_prime_pullback_profile import (  # noqa: E402
    DEFAULT_ADV_WINDOW,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run prime-only speculative volume-surge pullback-profile research "
            "and persist a reproducible bundle under ~/.local/share/trading25/research/."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        ...
        help="Maximum session offset used to find the deepest pullback label.",
    )
    parser.add_argument(
        "--future-horizons",
        type=_parse_int_csv,
        default=DEFAULT_FUTURE_HORIZONS,
        help="Comma-separated future horizons measured from the deepest pullback date.",
    )
    parser.add_argument(
        "--primary-horizon",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Positive/negative example count retained for the primary horizon.",
    )

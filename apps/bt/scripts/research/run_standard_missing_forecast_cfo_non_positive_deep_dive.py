#!/usr/bin/env python3
"""Runner-first entrypoint for market forecast-missing & CFO<=0 deep dive."""
...
from src.domains.analytics.standard_missing_forecast_cfo_non_positive_deep_dive import (  # noqa: E402
    DEFAULT_HORIZONS,
    DEFAULT_MARKET,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Deep-dive market-scoped FY actual EPS<0 events where forecast is missing "
            "and OperatingCashFlow is non-positive."
        )
    )
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        "--market",
        choices=["prime", "standard", "topix500", "primeExTopix500"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Trailing session count used in the base market negative-EPS decomposition.",
    )
    parser.add_argument(
        "--prior-sessions",
        type=int,

"""Run the TOPIX100 Top1 open-to-open 5D fixed committee overlay study."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the TOPIX100 Top1 open-to-open 5D stock book with the fixed "
            "TOPIX 4-member committee overlay and persist a reproducible bundle."
        )
    )
    ...
    parser.add_argument("--top1-model-name", default=DEFAULT_TOP1_MODEL_NAME)
    ...
    parser.add_argument(
        "--allow-incomplete-trades",
        action="store_true",
        help="Drop incomplete reconstructed trade paths instead of failing.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--committee-trend-vote-threshold",
        type=int,
        default=1,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_topix100_top1_open_to_open_5d_fixed_committee_overlay_research(
        ...
        top1_model_name=args.top1_model_name,
        top_k=args.top_k,
        sleeve_count=args.sleeve_count,
        holding_session_count=args.holding_session_count,
        require_complete_trades=not args.allow_incomplete_trades,

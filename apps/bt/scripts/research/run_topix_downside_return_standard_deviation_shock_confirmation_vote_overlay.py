"""Run TOPIX shock-confirmation vote overlay research."""
...
from src.domains.analytics.topix_downside_return_standard_deviation_shock_confirmation_vote_overlay import (  # noqa: E402
    DEFAULT_BREADTH_FAMILY_RULES,
    DEFAULT_BREADTH_VOTE_THRESHOLDS,
...
DEFAULT_BREADTH_FAMILY_RULE_SPEC = ",".join(DEFAULT_BREADTH_FAMILY_RULES)
DEFAULT_TREND_VOTE_THRESHOLD_SPEC = ",".join(str(value) for value in DEFAULT_TREND_VOTE_THRESHOLDS)
DEFAULT_BREADTH_VOTE_THRESHOLD_SPEC = ",".join(
    str(value) for value in DEFAULT_BREADTH_VOTE_THRESHOLDS
)
DEFAULT_CONFIRMATION_MODE_SPEC = ",".join(DEFAULT_CONFIRMATION_MODES)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run TOPIX shock-confirmation vote overlay research and persist a "
    ...
    parser.add_argument("--breadth-family-rules", default=DEFAULT_BREADTH_FAMILY_RULE_SPEC)
    parser.add_argument("--trend-vote-thresholds", default=DEFAULT_TREND_VOTE_THRESHOLD_SPEC)
    parser.add_argument(
        "--breadth-vote-thresholds",
        default=DEFAULT_BREADTH_VOTE_THRESHOLD_SPEC,
    )
    parser.add_argument("--confirmation-modes", default=DEFAULT_CONFIRMATION_MODE_SPEC)
    parser.add_argument(
    ...
    args.breadth_family_rules = _parse_string_sequence(args.breadth_family_rules)
    args.trend_vote_thresholds = _parse_positive_int_sequence(args.trend_vote_thresholds)
    args.breadth_vote_thresholds = _parse_positive_int_sequence(args.breadth_vote_thresholds)
    args.confirmation_modes = _parse_string_sequence(args.confirmation_modes)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_topix_downside_return_standard_deviation_shock_confirmation_vote_overlay_research(
        ...
        breadth_family_rules=args.breadth_family_rules,
        trend_vote_thresholds=args.trend_vote_thresholds,
        breadth_vote_thresholds=args.breadth_vote_thresholds,

#!/usr/bin/env python3
"""Runner-first entrypoint for volume-ratio future-return regime research."""
...
from src.domains.analytics.volume_ratio_future_return_regime import (  # noqa: E402
    DEFAULT_HORIZONS,
    DEFAULT_LOOKBACK_YEARS,
    DEFAULT_LONG_WINDOWS,
    DEFAULT_MIN_SIGNAL_EVENTS,
    DEFAULT_MIN_UNIQUE_CODES,
    DEFAULT_SAMPLE_EVENT_SIZE_PER_UNIVERSE,
    DEFAULT_SAMPLE_SEED,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run multi-universe volume-ratio future-return regime research and "
    ...
    parser.add_argument(
        ...
        help="Comma-separated short volume MA windows.",
    )
    parser.add_argument(
        "--long-windows",
        type=_parse_int_csv,
        default=DEFAULT_LONG_WINDOWS,
        help="Comma-separated long volume MA windows.",
    )
    parser.add_argument(
        "--thresholds",
        type=_parse_float_csv,
        default=DEFAULT_THRESHOLDS,
        help="Comma-separated volume-ratio thresholds.",
    )
    parser.add_argument(
        "--horizons",
        type=_parse_int_csv,
    ...
    parser.add_argument(
        ...
        help="Seed token for deterministic code/event samples.",
    )
    parser.add_argument(
        "--sample-size-per-universe",
        type=int,
        default=DEFAULT_SAMPLE_SIZE_PER_UNIVERSE,
        help="Deterministic sample size per universe for code inspection.",
    )
    parser.add_argument(
        "--sample-event-size-per-universe",
        type=int,
        default=DEFAULT_SAMPLE_EVENT_SIZE_PER_UNIVERSE,
        help="Deterministic reference-signal event sample size per universe.",
    )
    parser.add_argument(
        "--min-signal-events",
        type=int,
        default=DEFAULT_MIN_SIGNAL_EVENTS,
        help="Minimum signal events required for the stability gate.",
    )
    parser.add_argument(
        "--min-unique-codes",
        type=int,
        default=DEFAULT_MIN_UNIQUE_CODES,
        help="Minimum unique codes required for the stability gate.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=DEFAULT_TOP_K,
        help="Top validated configs to retain per universe / entry mode / horizon.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_volume_ratio_future_return_regime_research(
        ...
        sample_event_size_per_universe=args.sample_event_size_per_universe,

...
class RetainedPromotionService:
    ...
    def __init__(
        ...
        *,
        journal_factory: type[PromotionJournal] = PromotionJournal,
    ) -> None:
        ...
        self._promotion_evidence = PromotionEvidenceService(
            workspace,
            evidence,
            market_identity,
            eligibility,
        )
        self._promotion_reports = PromotionReportService(
            workspace,
            market_identity,
        ...
        self._cleanup = PromotionCleanupService(
            workspace,
            market_identity,
            self._promotion_evidence,
            self._promotion_reports,
        )
        self._recovery = PromotionRecoveryService(
            workspace,
            self._promotion_evidence,
            self._promotion_reports,
            self._cleanup,
        )
        self._rollback = PromotionRollbackService(
            workspace,
            backups,
            self._promotion_evidence,
            self._promotion_reports,
        ...
        self._artifacts = PromotionArtifactService(
            workspace,
            market_identity,
            backups,
            eligibility,
        ...
        self._transaction = PromotionTransactionService(
            workspace,
            market_identity,
            reports,
            runtime_smoke,
    ...
    def _recover_retained_promotion(
        ...
        *,
        ...
        with self._workspace.existing_exclusive_operation():
            ...
            with _market_operation_lease.MarketOperationLease.acquire_existing(
                ...
                try:
                    journal = self._journal_factory(
                        self._workspace.managed(), report_id, now=self._workspace.now
                    )
                    ...
                    if last.state is PromotionState.ROLLED_BACK:
                        self._recovery._validate_rolled_back_promotion_recovery(
                            report_id=report_id,
                    ...
                    if last.state is PromotionState.COMMITTED:
                        return self._recovery._validate_committed_promotion_recovery(
                            report_id=report_id,
                            retained_report_id=retained_report_id,
                            backup_id=backup_id,
                            base=base,
    ...
    def _recover_retained_promotion_preparation(
        ...
        *,
        ...
        holding_root = self._workspace.operations_root / "holding" / report_id
        ...
        *,
        ...
        if holding_location is None:
            ...
            try:
                ...
                return self._promotion_evidence._directory_identity_evidence(holding_fd)
            finally:
                os.close(holding_fd)
        ...
        opened: list[int] = []
        for candidate in (holding_root, self._cleanup._cleanup_staging_root(report_id)):
            try:
                opened.append(
    ...
    def _validate_recovered_holding(
        ...
        *,
        ...
        try:
            ...
            expected = {artifact.name: artifact for artifact in detached_artifacts}
            invalid = (
                self._promotion_evidence._directory_identity_evidence(holding_fd) != holding_directory
                or any(
                    artifact.name not in expected or artifact != expected[artifact.name]
                    for artifact in current_artifacts
                )
    ...
    def _retained_promotion_attempt_exists(self, report_id: str) -> bool:
        ...
        with self._workspace.managed_root_scope():
            for candidate in candidates:
                try:
                    self._workspace.managed().stat(candidate)
                except FileNotFoundError:
                    continue
                return True
        return False
    ...
    def _promote_retained_under_leases(
        ...
        *,
        ...
        try:
            return self._transaction._promote_retained_under_leases_unchecked(
                preparation,
                journal=journal,
                config=config,

...
class PromotionRecoveryMixin:
    def _validate_committed_promotion_recovery(
        ...
        *,
        ...
        try:
            report = json.loads(
                self._managed().read_bytes(self._managed_relative(report_path))
            )
        ...
        expected_quarantine_value = self._managed_relative(quarantine_path).as_posix()
        ...
        marker_path = self.operations_root / "consumed" / f"{retained_report_id}.json"
        ...
        if not isinstance(cleanup, dict) or not (
            ...
            == self._managed_relative(self._cleanup_staging_root(report_id)).as_posix()
            and cleanup.get("cleanupControlPath")
            == self._managed_relative(self._cleanup_control_path(report_id)).as_posix()
            and cleanup.get("cleanupResultPath")
            == self._managed_relative(self._cleanup_result_path(report_id)).as_posix()
    ...
    def _validate_rolled_back_promotion_recovery(
        ...
        *,
        ...
        active = self._market_location_identity(self._active_lease_fd_root())
        retained = self._payload_location_identity(retained_root / "market-timeseries")

...
from .promotion_artifacts import PromotionArtifactService
...
class PromotionTransactionService:
    def __init__(
        self,
        ...
        artifacts: PromotionArtifactService,
    ) -> None:
    ...
    def _promote_retained_under_leases_unchecked(
        ...
        *,
        ...
        retained_relative = self._workspace._managed_relative(retained_market)
        quarantine = self._workspace.operations_root / "quarantine" / operation_id
        runtime_name = f".cutover-runtime-{operation_id}"
        report_dir = self._workspace.operations_root / "reports" / operation_id
    ...
    def _exchange_and_quarantine_retained_market(
        ...
        *,
        ...
        self._workspace._promotion_boundary_hook("exchange_fsynced")
        ...
        exchanged = self._promotion_reports._promotion_identities(
            base,
        ...
        self._promotion_evidence._append_preparation_state(journal, PromotionState.EXCHANGED, exchanged)
        self._workspace._promotion_boundary_hook("exchanged_journaled")
        ...
        self._workspace._promotion_boundary_hook("quarantine_fsynced")
        quarantine_location = self._promotion_reports._payload_location_identity(quarantine)
        if (
        ...
        quarantined = self._promotion_reports._promotion_identities(
            base,
            active_current=active_location,
        ...
        self._workspace._promotion_boundary_hook("quarantined_journaled")
    ...
    def _run_promoted_market_smoke(
        ...
        *,
        ...
        environment = self._promotion_reports._promotion_runtime_environment(
        ...
        try:
            api = self._workspace.runtime.start(
                ...
                market_fd=market_fd,
                lease_fd=self._workspace._active_lease.fd,
                retained_lease_fd=self._workspace._retained_lease.fd,
        ...
        try:
            ...
            os.close(market_fd)
        self._workspace._promotion_boundary_hook("smoke_joined")
        active_runtime_market_fd = self._workspace.managed().open_dir(Path("market-timeseries"))
        try:
            self._workspace._remove_market_runtime(active_runtime_market_fd, runtime_name)
        finally:
            os.close(active_runtime_market_fd)
        log_bytes = self._workspace.managed().read_bytes(self._workspace._managed_relative(log_path))
        ...
        active_after = self._promotion_evidence._market_location_identity(self._promotion_evidence._active_lease_fd_root())
        ...
        smoke_passed = self._promotion_reports._promotion_identities(
            base,
        ...
        self._workspace._promotion_boundary_hook("smoke_journaled")
    ...
    def _commit_retained_promotion(
        ...
        *,
        ...
        self._workspace._require_unchanged_code_identity(code_version)
        cleanup_staging = self._cleanup._stage_held_promotion_artifacts(
            preparation,
            operation_id=operation_id,
        )
        self._workspace._promotion_boundary_hook("held_cleanup_fsynced")
        cleanup_location = {
            "directory": preparation.holding_directory_identity,
            "payload": base.retained_v4_payload,
        }
        cleanup_staged = self._promotion_reports._promotion_identities(
            base,
            active_current=active_after,
            retained_current=None,
            quarantine_current=quarantine_location,
        ...
        active_market_fd = self._workspace.managed().open_dir(Path("market-timeseries"))
        try:
            self._artifacts._validate_canonical_market_payload(active_market_fd)
        finally:
            os.close(active_market_fd)

        created_at = self._workspace.now()
        ...
        report = self._promotion_reports._build_retained_promotion_report(expectation)

        def final_validator() -> None:
            current_active = self._promotion_evidence._market_location_identity(
                self._promotion_evidence._active_lease_fd_root()
            )
            current_quarantine = self._promotion_reports._payload_location_identity(quarantine)
            current_expectation = self._promotion_reports._retained_promotion_report_expectation(
                operation_id=operation_id,
                created_at=created_at,
                code_version=code_version,
                preparation=preparation,
            ...
            staging_fd = self._workspace.managed().open_dir(
                self._workspace._managed_relative(cleanup_staging)
            )
            try:
                if (
        ...
        self._workspace._promotion_boundary_hook("report_fsynced")
        persisted = self._promotion_reports._promotion_identities(
            base,
            active_current=active_after,
            retained_current=None,
            quarantine_current=quarantine_location,
        ...
        self._workspace._promotion_boundary_hook("report_journaled")
        self._cleanup._write_source_consumed_marker(
            retained_report_id=eligibility.retained_report_id,
            operation_id=operation_id,
            promotion_report_sha256=self._workspace._sha256(report_path),
        )
        self._workspace._promotion_boundary_hook("consumed_marker_fsynced")
        committed = self._promotion_reports._promotion_identities(
            base,
            active_current=active_after,
            retained_current=None,
            quarantine_current=quarantine_location,
        ...
        self._workspace._promotion_boundary_hook("committed_journaled")
    ...
    def _retained_promotion_eligibility_scope(
        ...
        *,
        ...
        with self._workspace.existing_exclusive_operation() as code_version:
            ...
            with _market_operation_lease.MarketOperationLease.acquire_existing(
                ...
                self._workspace._retained_lease = retained_lease
                try:
                    eligibility = (
                        self._promotion_evidence._validate_retained_promotion_eligibility_under_leases(
                            report_id=report_id,
                    ...
                    if (
                        self._market_identity.market_tree_identity(self._promotion_evidence._active_lease_fd_root())
                    ...
                    if (
                        self._market_identity.market_tree_identity(retained_lease.root_fd)
                    ...
                    self._workspace._retained_lease = None

...
class CutoverWorkspace:
    ...
    @contextmanager
    def managed_root_scope(self) -> Iterator[_managed_root.ManagedRootFd]:
    ...
    def managed(self) -> _managed_root.ManagedRootFd:
    ...
    @contextmanager
    def existing_exclusive_operation(self) -> Iterator[str]:
        if self._active_lease is not None:
            raise _managed_root.CutoverSafetyError(
    ...
    def _assert_managed_directory(self, path: Path) -> Path:
        ...
        if path == self.data_root:
            os.fstat(self.managed().fd)
            return path
        fd = self.managed().open_dir(self._managed_relative(path))
        os.close(fd)
        return path
    ...
    def _assert_managed_target_absent(self, path: Path) -> Path:
        path = self._managed_path(path)
        parent, name = self.managed().open_parent(self._managed_relative(path))
        try:
    ...
    def _secure_rename(self, source: Path, target: Path) -> None:
        source_parent, source_name = self.managed().open_parent(
            self._managed_relative(source)
        )
        target_parent, target_name = self.managed().open_parent(
            self._managed_relative(target)
        )
        try:
            self._rename_at_hook(source, target)
            current_source_parent, _ = self.managed().open_parent(
                self._managed_relative(source)
            )
            try:
                current_target_parent, _ = self.managed().open_parent(
                    self._managed_relative(target)
                )
                try:
    ...
    def _assert_current_data_root_identity(self) -> None:
        retained = os.fstat(self.managed().fd)
        try:
    ...
    def _assert_managed_directory_identity(
        ...
        current_fd = self.managed().open_dir(self._managed_relative(path))
        try:
    ...
    def _market_identity_guard(self) -> Iterator[int]:
        ...
        retained = self.managed().open_dir(market_relative)
        try:
            retained_stat = os.fstat(retained)
            ...
            yield retained
            current = self.managed().open_dir(market_relative)
            try:
                current_stat = os.fstat(current)
    ...
    def _source_files(self, root: Path) -> list[Path]:
        if self._managed_root_fd is not None:
            try:
                ...
                files: list[Path] = []
                for relative, entry_stat in self.managed().regular_files(
                    root_relative
                ):
                    if (
    ...
    def _write_managed_file_create(self, target: Path, payload: bytes) -> None:
        ...
        try:
            ...
            os.close(fd)
            self.managed().unlink(relative, missing_ok=True)
            raise
        os.close(fd)
    ...
    def _copy_regular_to_managed(self, source: Path, target: Path) -> tuple[int, str]:
        try:
            ...
            source_fd = self.managed().open_regular(source_relative, os.O_RDONLY)
        ...
        try:
            ...
            target_fd = self.managed().open_regular(
                target_relative,
                os.O_CREAT | os.O_EXCL | os.O_WRONLY,
            )
            try:
                ...
                os.close(target_fd)
                self.managed().unlink(target_relative, missing_ok=True)
                raise
            os.close(target_fd)
        finally:
            os.close(source_fd)

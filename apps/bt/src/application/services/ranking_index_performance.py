...
_LONG_SECTOR_LEADERSHIP_HISTORY_CALENDAR_DAYS = 1700
_SECTOR_INDEX_MAP_SQL = """
            VALUES
                ('水産･農林業', '0040'),
                ('鉱業', '0041'),
                ('建設業', '0042'),
...
def _long_sector_leadership_history_start_date(target_date: str) -> str:
    parsed = datetime.strptime(target_date, "%Y-%m-%d").date()
    return (parsed - timedelta(days=_LONG_SECTOR_LEADERSHIP_HISTORY_CALENDAR_DAYS)).isoformat()
...
def load_long_sector_leadership_by_name(
    reader: MarketDbReader,
    *,
    table_exists: Callable[[str], bool],
    date: str,
    ...
    prefer_price_code = prefer_4digit_order_sql("code")
    prefer_master_code = prefer_4digit_order_sql("m.code")
    history_start_date = _long_sector_leadership_history_start_date(date)
    ...
    rows = reader.query(
        ...
        topix_points AS (
            SELECT
                MAX(CASE WHEN rn = 1 THEN close END) AS current_close,
                MAX(CASE WHEN rn = 121 THEN close END) AS close_120d,
                MAX(CASE WHEN rn = 253 THEN close END) AS close_252d,
                MAX(CASE WHEN rn = 505 THEN close END) AS close_504d
            FROM topix_ranked
        ),
        sector_index_map(sector_33_name, sector_index_code) AS (
            {_SECTOR_INDEX_MAP_SQL}
        ...
        sector_index_metrics AS (
            SELECT
                sector_33_name,
                sector_index_code,
                ((sip.current_close / sip.close_120d) - 1.0) * 100.0
                    - ((t.current_close / t.close_120d) - 1.0) * 100.0
                    AS sector_index_120d_topix_excess_pct,
                ((sip.current_close / sip.close_252d) - 1.0) * 100.0
                    - ((t.current_close / t.close_252d) - 1.0) * 100.0
                    AS sector_index_252d_topix_excess_pct,
                ((sip.current_close / sip.close_504d) - 1.0) * 100.0
                    - ((t.current_close / t.close_504d) - 1.0) * 100.0
                    AS sector_index_504d_topix_excess_pct
            FROM sector_index_points sip
            CROSS JOIN topix_points t
            WHERE sip.current_close IS NOT NULL
                AND sip.close_120d IS NOT NULL
                AND sip.close_252d IS NOT NULL
                AND sip.close_504d IS NOT NULL
                AND sip.close_120d > 0
                AND sip.close_252d > 0
                AND sip.close_504d > 0
                AND t.current_close IS NOT NULL
                AND t.close_120d IS NOT NULL
                AND t.close_252d IS NOT NULL
                AND t.close_504d IS NOT NULL
                AND t.close_120d > 0
                AND t.close_252d > 0
                AND t.close_504d > 0
        ),
        ...
        stock_returns AS (
            SELECT
                m.sector_33_name,
                p.normalized_code,
                ((p.current_close / p.close_120d) - 1.0) * 100.0
                    - ((t.current_close / t.close_120d) - 1.0) * 100.0 AS excess_120d,
                ((p.current_close / p.close_252d) - 1.0) * 100.0
                    - ((t.current_close / t.close_252d) - 1.0) * 100.0 AS excess_252d,
                ((p.current_close / p.close_504d) - 1.0) * 100.0
                    - ((t.current_close / t.close_504d) - 1.0) * 100.0 AS excess_504d
            FROM price_points p
            JOIN stock_master m
                ON m.normalized_code = p.normalized_code
            CROSS JOIN topix_points t
            WHERE p.current_close IS NOT NULL
                AND p.close_120d IS NOT NULL
                AND p.close_252d IS NOT NULL
                AND p.close_504d IS NOT NULL
                AND p.close_120d > 0
                AND p.close_252d > 0
                AND p.close_504d > 0
                AND t.current_close IS NOT NULL
        ...
        sector_metrics AS (
            SELECT
                sector_33_name,
                COUNT(*) AS stock_count,
                AVG(excess_120d) AS sector_120d_topix_excess_pct,
                AVG(excess_252d) AS sector_252d_topix_excess_pct,
                AVG(excess_504d) AS sector_504d_topix_excess_pct,
                AVG(CASE WHEN excess_120d > 0 THEN 1.0 ELSE 0.0 END) * 100.0
                    AS sector_breadth_120d_pct,
                AVG(CASE WHEN excess_252d > 0 THEN 1.0 ELSE 0.0 END) * 100.0
                    AS sector_breadth_252d_pct,
                AVG(CASE WHEN excess_504d > 0 THEN 1.0 ELSE 0.0 END) * 100.0
                    AS sector_breadth_504d_pct
            FROM stock_returns
            GROUP BY sector_33_name
        ),
        ranked_metrics AS (
            SELECT
                sm.*,
                sim.sector_index_code,
                sim.sector_index_120d_topix_excess_pct,
                sim.sector_index_252d_topix_excess_pct,
                sim.sector_index_504d_topix_excess_pct,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sim.sector_index_120d_topix_excess_pct)
                END AS rank_index_120d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sim.sector_index_252d_topix_excess_pct)
                END AS rank_index_252d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sim.sector_index_504d_topix_excess_pct)
                END AS rank_index_504d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_120d_topix_excess_pct)
                END AS rank_constituent_120d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_252d_topix_excess_pct)
                END AS rank_constituent_252d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_504d_topix_excess_pct)
                END AS rank_constituent_504d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_breadth_120d_pct)
                END AS rank_breadth_120d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_breadth_252d_pct)
                END AS rank_breadth_252d,
                CASE
                    WHEN COUNT(*) OVER () = 1 THEN 0.5
                    ELSE PERCENT_RANK() OVER (ORDER BY sm.sector_breadth_504d_pct)
                END AS rank_breadth_504d
            FROM sector_metrics sm
            JOIN sector_index_metrics sim
              ON sim.sector_33_name = sm.sector_33_name
        ),
        scored_metrics AS (
            SELECT
                *,
                (rank_index_120d + rank_index_252d + rank_index_504d) / 3.0
                    AS long_index_leadership_score,
                (
                    rank_constituent_120d
                    + rank_constituent_252d
                    + rank_constituent_504d
                    + rank_breadth_120d
                    + rank_breadth_252d
                    + rank_breadth_504d
                ) / 6.0 AS long_constituent_breadth_leadership_score
            FROM ranked_metrics
        )
        SELECT
            sector_33_name,

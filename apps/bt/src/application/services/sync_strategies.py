...
async def _resolve_incremental_stock_master_dates(ctx: SyncContext) -> list[str]:
    missing_dates = await asyncio.to_thread(
        ctx.market_db.get_missing_stock_master_dates,
        limit=None,
    )
    pending_frontier_dates = await asyncio.to_thread(
        ctx.market_db.get_stock_master_pending_frontier_dates
    )
    return sorted(set(missing_dates) | pending_frontier_dates, key=_date_sort_key)

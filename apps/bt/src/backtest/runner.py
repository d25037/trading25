...
class BacktestRunner:
    ...
    def execute(
        ...
        # Executor初期化（戦略設定のexecution設定を尊重）
        ...
        template_path = str(
            self.config_loader.get_template_notebook_path(strategy_config)
        )
    ...
    def _run_walk_forward(self, parameters: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # NOTE: 1銘柄目のインデックスでsplit生成（全銘柄同一日付範囲が前提）
        try:
    ...
    @staticmethod
    def _aggregate_walk_forward_metrics(results: list[dict[str, Any]]) -> dict[str, float]:
    ...
    def _build_parameters(
        ...
        if config_override:
            ...
            # マージ後のシグナルパラメータをPydanticでバリデーション

...
def _build_portfolio_daily_df(
    ...
    for event in selected_event_df.to_dict(orient="records"):
        ...
        if path_df.empty:
            continue
        try:
            entry_open_value = float(cast(float, event.get("entry_open")))
        ...
        group_key = tuple(event[column] for column in _BREAKOUT_GROUP_COLUMNS)
    ...
    records = [
        {
            **dict(zip(_BREAKOUT_GROUP_COLUMNS, key[:-1], strict=True)),
            "date": key[-1],
    ...
    daily_df = pd.DataFrame(records).sort_values(
        [*_BREAKOUT_GROUP_COLUMNS, "date"],
        kind="stable",
    ).reset_index(drop=True)
    daily_df["portfolio_value"] = np.nan
    daily_df["drawdown_pct"] = np.nan
    for _, group in daily_df.groupby(list(_BREAKOUT_GROUP_COLUMNS), observed=True, sort=False):

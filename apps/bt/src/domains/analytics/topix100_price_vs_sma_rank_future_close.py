"""
TOPIX100 price-vs-SMA rank / future close research analytics.
...
def _build_price_feature_name(window: int) -> str:
    return f"price_vs_sma_{window}_gap"


def _build_price_feature_label(window: int) -> str:
...
PRICE_FEATURE_LABEL_MAP: dict[str, str] = {
...
class Topix100PriceVsSmaRankFutureCloseResearchResult:
    db_path: str
...
def _normalize_price_sma_windows(
    price_sma_windows: tuple[int, ...] | list[int] | None,
) -> tuple[int, ...]:
    if price_sma_windows is None:
        return PRICE_SMA_WINDOW_ORDER
    requested = list(dict.fromkeys(int(window) for window in price_sma_windows))
    unsupported = sorted(set(requested) - set(PRICE_SMA_WINDOW_ORDER))
    if unsupported:
        raise ValueError(
            "Unsupported price_sma_windows: "
            f"{unsupported}. Supported windows are {list(PRICE_SMA_WINDOW_ORDER)}."
        )
    normalized = tuple(
        window for window in PRICE_SMA_WINDOW_ORDER if window in requested
    )
    if not normalized:
        raise ValueError("price_sma_windows must include at least one SMA window.")
    return normalized
...
def _sort_frame(
    df: pd.DataFrame,
    *,
    known_feature_order: tuple[str, ...],
) -> pd.DataFrame:
    if df.empty:
        return df

    sorted_df = df.copy()
    added_ranking_feature = False
    if "ranking_feature" not in sorted_df.columns and "price_feature" in sorted_df.columns:
        sorted_df["ranking_feature"] = sorted_df["price_feature"]
        added_ranking_feature = True

    sorted_df = _base_sort_frame(sorted_df, known_feature_order=list(known_feature_order))
    ...
    if added_ranking_feature:
        drop_columns.append("ranking_feature")
    return sorted_df.drop(columns=drop_columns)


def get_topix100_price_vs_sma_rank_future_close_available_date_range(
    db_path: str,
...
def _enrich_event_panel(
    ...
    *,
    ...
    price_feature_order = tuple(_build_price_feature_name(window) for window in price_sma_windows)
    ...
    for window in price_sma_windows:
        close_sma_cache[window] = _rolling_mean(panel, column_name="close", window=window)
        panel[_build_price_feature_name(window)] = (
...
def _build_ranked_panel(
    ...
    *,
    ...
    for price_feature in price_feature_order:
        ...
        ranked_panel_df["ranking_feature"] = price_feature
        ranked_panel_df["ranking_feature_label"] = PRICE_FEATURE_LABEL_MAP[price_feature]
        ranked_panel_df["ranking_value"] = event_panel_df[price_feature].astype(float)
        frames.append(
            _assign_feature_deciles(
                ranked_panel_df,
                known_feature_order=list(price_feature_order),
            )
        )
    return _sort_frame(pd.concat(frames, ignore_index=True), known_feature_order=price_feature_order)


def _build_price_bucket_daily_means(
    ...
    *,
    ...
    return _sort_frame(daily_means_df, known_feature_order=price_feature_order)


def _summarize_price_buckets(
    price_bucket_daily_means_df: pd.DataFrame,
    *,
    price_feature_order: tuple[str, ...],
) -> pd.DataFrame:
    ...
    return _sort_frame(summary_df, known_feature_order=price_feature_order)
...
def _aligned_price_bucket_pivot(
    ...
    *,
    ...
    scoped_df = price_bucket_daily_means_df.loc[
        (price_bucket_daily_means_df["ranking_feature"] == ranking_feature)
...
def _build_price_bucket_pairwise_significance(
    ...
    *,
    ...
    for ranking_feature in price_feature_order:
                ...
                if pivot_df.empty:
                    for left_bucket, right_bucket in combinations(PRICE_BUCKET_ORDER, 2):
                        records.append(
                            {
                                "ranking_feature": ranking_feature,
                                "ranking_feature_label": PRICE_FEATURE_LABEL_MAP[ranking_feature],
                                "horizon_key": horizon_key,
        ...
        for horizon_key in HORIZON_ORDER:
            for metric_key in METRIC_ORDER:
                scoped_df = price_bucket_pairwise_significance_df[
                    (price_bucket_pairwise_significance_df["ranking_feature"] == ranking_feature)
...
def _build_price_volume_split_panel(
    ...
    *,
    ...
    return _sort_frame(
        ...
        known_feature_order=price_feature_order,
    )
...
def _build_price_volume_daily_means(
    ...
    *,
    ...
    return _sort_frame(daily_group_means_df, known_feature_order=price_feature_order)
...
def run_topix100_price_vs_sma_rank_future_close_research(
    db_path: str,
    *,
    ...
    price_bucket_daily_means_df = _build_price_bucket_daily_means(
        horizon_panel_df,
        price_feature_order=price_feature_order,
    )
    ...
    return Topix100PriceVsSmaRankFutureCloseResearchResult(
        ...
        price_bucket_summary_df=_summarize_price_buckets(
            price_bucket_daily_means_df,
            price_feature_order=price_feature_order,
        ),

...
def _ordered_feature_values(values: list[str] | pd.Series) -> list[str]:
    ...
    known_values = [value for value in RANKING_FEATURE_ORDER if value in unique_values]
...
def _assign_feature_deciles(ranked_panel_df: pd.DataFrame) -> pd.DataFrame:
    ...
    _group_count = len(QUARTILE_ORDER)
    ranked_panel_df["feature_quartile_index"] = (
        ((ranked_panel_df["feature_rank_desc"] - 1) * _group_count)
    ...
    ranked_panel_df["feature_quartile"] = ranked_panel_df["feature_quartile_index"].map(
        {index: f"Q{index}" for index in range(1, _group_count + 1)}
    )
    ranked_panel_df["feature_quartile_label"] = ranked_panel_df["feature_quartile"].map(
        _QUARTILE_LABEL_MAP
    )
    return ranked_panel_df.reset_index(drop=True)
    ...
    if "ranking_feature" in sorted_df.columns:
        feature_order = {
            ...
            for index, key in enumerate(
                _ordered_feature_values(sorted_df["ranking_feature"]),
                start=1,
            )
        }
...
def _ranking_feature_label_lookup(df: pd.DataFrame) -> dict[str, str]:
    ...
    if "ranking_feature_label" not in scoped_df.columns:
        return {
            feature: _RANKING_FEATURE_LABEL_MAP.get(feature, feature)
...
def _build_global_significance(daily_group_means_df: pd.DataFrame) -> pd.DataFrame:
    ...
    feature_values = _ordered_feature_values(daily_group_means_df["ranking_feature"])
    ...
    for ranking_feature in feature_values:
        for horizon_key in HORIZON_ORDER:
            for metric_key in METRIC_ORDER:
                ...
                samples = [
                    pivot_df[quartile].to_numpy(dtype=float)
    ...
    for ranking_feature in feature_values:
        for horizon_key in HORIZON_ORDER:
            for metric_key in METRIC_ORDER:
                ...
                if pivot_df.empty:
                    for left_quartile, right_quartile in combinations(
                        QUARTILE_ORDER,
                        2,
                    ):
                        records.append(

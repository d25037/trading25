"""
Walk-forward validation for TOPIX100 streak 3/53 next-session open-to-close 10D LightGBM.
...
from src.domains.analytics.topix100_streak_353_next_session_open_to_close_10d_lightgbm import (
    ...
    build_feature_panel_from_state_event_df,
)
...
@dataclass(frozen=True)
class Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult:
    db_path: str
...
def run_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_research(
    db_path: str,
    *,
...
) -> Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult:
...
def _build_research_feature_panel_df(
    *,
    ...
    feature_panel_df = build_feature_panel_from_state_event_df(
...
def _run_walkforward_from_panel(
    *,
    ...
    return Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult(
        db_path=db_path,
...
def _build_walkforward_prediction_artifacts(
    *,
    ...
    benchmark_daily_df = _load_topix_open_to_close_10d_benchmark_df(
        db_path,
    ...
    for split_index, split in enumerate(splits, start=1):
        ...
        baseline_lookup_df = _build_baseline_lookup_df(
            ...
            target_column="next_session_open_to_close_10d_return",
        )
...
def write_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_research_bundle(
    result: Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult,
    *,
    ...
    return write_dataclass_research_bundle(
        experiment_id=TOPIX100_STREAK_353_NEXT_SESSION_OPEN_TO_CLOSE_10D_LIGHTGBM_WALKFORWARD_EXPERIMENT_ID,
        module=__name__,
        function="run_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_research",
        params={
...
def load_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_research_bundle(
    bundle_path: str | Path,
) -> Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult:
    return load_dataclass_research_bundle(
        bundle_path,
        result_type=Topix100Streak353NextSessionOpenToClose10dLightgbmWalkforwardResearchResult,
        table_field_names=_RESULT_TABLE_NAMES,
    )


def get_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_latest_bundle_path(
    *,
    output_root: str | Path | None = None,
) -> Path | None:
...
def get_topix100_streak_353_next_session_open_to_close_10d_lightgbm_walkforward_bundle_path_for_run_id(
    run_id: str,
    *,
    output_root: str | Path | None = None,
) -> Path:
...
def _build_research_bundle_summary_markdown(
    ...
    lines = [
        ...
        "- Target: `X+1 open -> X+10 close return`",
        ...
        "## Current Read",
        "",
        "This study rebuilds a long-only lookup baseline and a LightGBM regressor inside each train block, then buys the top-ranked names at X+1 open and evaluates the realized X+10 close outcome only in the following out-of-sample block.",
    ]
    ...
    lines.extend(
        [
            ...
            "- `benchmark_daily_df`: signal-date-aligned TOPIX X+1 open -> X+10 close benchmark series",
...
def _build_published_summary_payload(
    ...
    result_bullets = [
        ...
        "The portfolio is long-only: buy the top-ranked names at X+1 open, hold until X+10 close, then compare raw return, excess vs TOPIX, and excess vs the equal-weight TOPIX100 universe.",
    ]
    if comparison_row is not None:
    ...
    return {
        "title": "TOPIX100 Streak 3/53 Next-Session Open-to-Close 10D LightGBM Walk-Forward",
        "tags": ["TOPIX100", "streaks", "lightgbm", "swing", "walk-forward"],
        "purpose": (
            "Record an invalidated TOPIX100 streak 3 / 53 swing score. The current bundle used current TOPIX100 membership on past dates, so the headline metrics must not be used until the universe is rebuilt point-in-time."
        ),
        "method": [
            "Build the TOPIX100 streak 3 / 53 daily state panel and target each signal date with X+1 open -> X+10 close return; this historical bundle is invalidated because TOPIX100 membership was not date-effective.",
        ...
        "selectedParameters": [
            ...
            {"label": "Target", "value": "X+10 close / X+1 open - 1"},
        ...
        "tableHighlights": [
            ...
            },
            {
                ...
                "description": "Signal-date-aligned X+1 open -> X+10 close benchmark return for TOPIX.",
            },
            {
...
def _load_topix_open_to_close_10d_benchmark_df(
    db_path: str,
    *,
    start_date: str,
    end_date: str,
            ...
            benchmark_df = conn.execute(
                ...
                WITH ranked_sessions AS (
                    SELECT
                        ...
                        LEAD(date, 10) OVER (ORDER BY date ASC) AS benchmark_exit_date,
                        LEAD(close, 10) OVER (ORDER BY date ASC) AS benchmark_exit_close
                    FROM topix_data
                )
                SELECT
                    date,
                    ...
                    benchmark_exit_date,
                    benchmark_exit_close / NULLIF(benchmark_entry_open, 0) - 1 AS topix_benchmark_return
            ...
            benchmark_df = conn.execute(
                ...
                ranked_sessions AS (
                    SELECT
                        ...
                        LEAD(id.date, 10) OVER (ORDER BY id.date ASC) AS benchmark_exit_date,
                        LEAD(id.close, 10) OVER (ORDER BY id.date ASC) AS benchmark_exit_close

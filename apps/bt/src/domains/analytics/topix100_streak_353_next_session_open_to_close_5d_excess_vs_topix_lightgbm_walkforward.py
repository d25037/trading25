...
TOPIX100_STREAK_353_NEXT_SESSION_OPEN_TO_CLOSE_5D_EXCESS_VS_TOPIX_LIGHTGBM_WALKFORWARD_EXPERIMENT_ID = (
    "market-behavior/topix100-streak-3-53-next-session-open-to-close-5d-excess-vs-topix-lightgbm-walkforward"
)
...
@dataclass(frozen=True)
class Topix100Streak353NextSessionOpenToClose5dExcessVsTopixLightgbmWalkforwardResearchResult:
    db_path: str
...
def run_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_research(
    db_path: str,
    *,
...
) -> Topix100Streak353NextSessionOpenToClose5dExcessVsTopixLightgbmWalkforwardResearchResult:
...
def _run_walkforward_from_panel(
    *,
    ...
    return Topix100Streak353NextSessionOpenToClose5dExcessVsTopixLightgbmWalkforwardResearchResult(
        db_path=db_path,
...
def _build_walkforward_prediction_artifacts(
    *,
    ...
    benchmark_daily_df = _load_topix_open_to_close_5d_benchmark_df(
        db_path,
        start_date=str(feature_panel_df["date"].min()),
        end_date=str(feature_panel_df["date"].max()),
    )
    feature_panel_df = feature_panel_df.merge(
        benchmark_daily_df[["date", "topix_benchmark_return"]],
        on="date",
        how="left",
        validate="many_to_one",
    )
    feature_panel_df[TARGET_COLUMN] = (
        feature_panel_df["next_session_open_to_close_5d_return"].astype(float)
        - feature_panel_df["topix_benchmark_return"].astype(float)
    )
    feature_panel_df = feature_panel_df.dropna(subset=[TARGET_COLUMN]).copy()
    if feature_panel_df.empty:
        raise ValueError("Feature panel was empty after aligning the TOPIX excess target")
...
def _build_baseline_validation_prediction_df(
    ...
    *,
    ...
    validation_df["realized_return"] = validation_df[evaluation_column].astype(float)
    validation_df["target_excess_return"] = validation_df[target_column].astype(float)
    base_columns = [
        "model_name",
        "date",
        "code",
...
def _build_lightgbm_validation_prediction_df(
    ...
    *,
    ...
    prediction_df["realized_return"] = validation_df[evaluation_column].astype(float)
    prediction_df["target_excess_return"] = validation_df[target_column].astype(float)
...
def write_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_research_bundle(
    ...
    *,
    ...
    return write_dataclass_research_bundle(
        experiment_id=TOPIX100_STREAK_353_NEXT_SESSION_OPEN_TO_CLOSE_5D_EXCESS_VS_TOPIX_LIGHTGBM_WALKFORWARD_EXPERIMENT_ID,
        module=__name__,
        function="run_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_research",
        params={
...
def load_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_research_bundle(
    bundle_path: str | Path,
) -> Topix100Streak353NextSessionOpenToClose5dExcessVsTopixLightgbmWalkforwardResearchResult:
    return load_dataclass_research_bundle(
        bundle_path,
        result_type=Topix100Streak353NextSessionOpenToClose5dExcessVsTopixLightgbmWalkforwardResearchResult,
        table_field_names=_RESULT_TABLE_NAMES,
    )


def get_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_latest_bundle_path(
    *,
    output_root: str | Path | None = None,
) -> Path | None:
...
def get_topix100_streak_353_next_session_open_to_close_5d_excess_vs_topix_lightgbm_walkforward_bundle_path_for_run_id(
    run_id: str,
    *,
    output_root: str | Path | None = None,
) -> Path:
...
def _build_research_bundle_summary_markdown(
    ...
    lines = [
        ...
        "- Target: `stock(X+1 open -> X+5 close) - TOPIX(X+1 open -> X+5 close)`",
        ...
        "## Current Read",
        "",
        "This study rebuilds a long-only lookup baseline and a LightGBM regressor inside each train block, learns directly on X+1 open -> X+5 close excess return versus TOPIX, then buys the top-ranked names in the following out-of-sample block.",
    ]
    ...
    lines.extend(
        [
            ...
            "- `benchmark_daily_df`: signal-date-aligned TOPIX X+1 open -> X+5 close benchmark series",
...
def _load_topix_open_to_close_5d_benchmark_df(
    db_path: str,
    *,
    start_date: str,
    end_date: str,
            ...
            benchmark_df = conn.execute(
                ...
                WITH ranked_sessions AS (
                    SELECT
                        ...
                        LEAD(date, 5) OVER (ORDER BY date ASC) AS benchmark_exit_date,
                        LEAD(close, 5) OVER (ORDER BY date ASC) AS benchmark_exit_close
                    FROM topix_data
                )
                SELECT
                    date,
            ...
            benchmark_df = conn.execute(
                ...
                ranked_sessions AS (
                    SELECT
                        ...
                        LEAD(id.date, 5) OVER (ORDER BY id.date ASC) AS benchmark_exit_date,
                        LEAD(id.close, 5) OVER (ORDER BY id.date ASC) AS benchmark_exit_close

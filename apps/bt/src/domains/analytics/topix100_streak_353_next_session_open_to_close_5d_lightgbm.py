...
- target is X+5 close / X+1 open - 1
"""
...
TOPIX100_STREAK_353_NEXT_SESSION_OPEN_TO_CLOSE_5D_LIGHTGBM_WALKFORWARD_EXPERIMENT_ID = (
    "market-behavior/topix100-streak-3-53-next-session-open-to-close-5d-lightgbm-walkforward"
)


def score_topix100_streak_353_next_session_open_to_close_5d_lightgbm_snapshot(
    db_path: str,
    *,
    target_date: str,
    ...
    if connection is not None:
        return _score_topix100_streak_353_next_session_open_to_close_5d_lightgbm_snapshot(
            db_path=db_path,
    ...
    return _score_topix100_streak_353_next_session_open_to_close_5d_lightgbm_snapshot_cached(
        db_path=db_path,
        target_date=target_date,
        price_feature=price_feature,
        volume_feature=volume_feature,
...
@lru_cache(maxsize=8)
def _score_topix100_streak_353_next_session_open_to_close_5d_lightgbm_snapshot_cached(
    db_path: str,
    target_date: str,
    price_feature: str,
    volume_feature: str,
...
def _score_topix100_streak_353_next_session_open_to_close_5d_lightgbm_snapshot(
    *,
    db_path: str,
    target_date: str,
    price_feature: str,
...
def _build_feature_panel_from_state_event_df(
    *,
    ...
    price_df["swing_exit_date"] = grouped["date"].shift(-5)
    price_df["swing_exit_close"] = grouped["close"].shift(-5).astype(float)
    price_df["next_session_open_to_close_5d_return"] = price_df["swing_exit_close"].div(
    ...
    merged_df = merged_df.dropna(subset=["next_session_open_to_close_5d_return"]).copy()
    if merged_df.empty:
    ...
    ordered_columns = [
        ...
        "swing_entry_date",
        "swing_exit_date",
        "next_session_open_to_close_5d_return",
    ]

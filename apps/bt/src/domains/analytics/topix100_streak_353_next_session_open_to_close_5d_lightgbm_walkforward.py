"""
Walk-forward validation for TOPIX100 streak 3/53 next-session open-to-close 5D LightGBM.
...
@dataclass(frozen=True)
class Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult:
    db_path: str
...
def run_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_research(
    db_path: str,
    *,
...
) -> Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult:
...
def _run_walkforward_from_panel(
    *,
    ...
    return Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult(
        db_path=db_path,
    ...
    *,
    ...
    for split_index, split in enumerate(splits, start=1):
        ...
        baseline_lookup_df = _build_baseline_lookup_df(
            ...
            target_column="next_session_open_to_close_5d_return",
        )
...
def write_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_research_bundle(
    result: Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult,
    *,
    ...
    return write_dataclass_research_bundle(
        experiment_id=TOPIX100_STREAK_353_NEXT_SESSION_OPEN_TO_CLOSE_5D_LIGHTGBM_WALKFORWARD_EXPERIMENT_ID,
        module=__name__,
        function="run_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_research",
        params={
...
def load_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_research_bundle(
    bundle_path: str | Path,
) -> Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult:
    return load_dataclass_research_bundle(
        bundle_path,
        result_type=Topix100Streak353NextSessionOpenToClose5dLightgbmWalkforwardResearchResult,
        table_field_names=_RESULT_TABLE_NAMES,
    )


def get_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_latest_bundle_path(
    *,
    output_root: str | Path | None = None,
) -> Path | None:
...
def get_topix100_streak_353_next_session_open_to_close_5d_lightgbm_walkforward_bundle_path_for_run_id(
    run_id: str,
    *,
    output_root: str | Path | None = None,
) -> Path:
...
def _build_research_bundle_summary_markdown(
    ...
    lines = [
        ...
        "- Target: `X+1 open -> X+5 close return`",
        ...
        "## Current Read",
        "",
        "This study rebuilds a long-only lookup baseline and a LightGBM regressor inside each train block, then buys the top-ranked names at X+1 open and evaluates the realized X+5 close outcome only in the following out-of-sample block.",
    ]
...
def _build_published_summary_payload(
    ...
    result_bullets = [
        ...
        "The portfolio is long-only: buy the top-ranked names at X+1 open, hold until X+5 close, then compare raw return, excess vs TOPIX, and excess vs the equal-weight TOPIX100 universe.",
    ]
    if comparison_row is not None:
    ...
    return {
        "title": "TOPIX100 Streak 3/53 Next-Session Open-to-Close 5D LightGBM Walk-Forward",
        ...
        "method": [
            "Build the TOPIX100 streak 3 / 53 daily state panel and target each signal date with X+1 open -> X+5 close return; this historical bundle is invalidated because TOPIX100 membership was not date-effective.",
        ...
        "selectedParameters": [
            ...
            {"label": "Target", "value": "X+5 close / X+1 open - 1"},

"""
TOPIX downside return-standard-deviation + trend + breadth overlay research.

This study keeps the same execution discipline as the plain downside-standard-
deviation overlay:
...
The difference is that a risk-off transition is only allowed when downside
return standard deviation expands *and* the move is confirmed by a weak TOPIX
trend signal plus weak TOPIX100 breadth.
"""
...
TOPIX_DOWNSIDE_RETURN_STANDARD_DEVIATION_TREND_BREADTH_OVERLAY_EXPERIMENT_ID = (
    "market-behavior/topix-downside-return-standard-deviation-trend-breadth-overlay"
)
...
_VALID_TREND_RULES = frozenset(DEFAULT_TREND_RULES)
_VALID_BREADTH_RULES = frozenset(DEFAULT_BREADTH_RULES)


@dataclass(frozen=True)
class TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult:
    db_path: str
    ...
    breadth_universe_membership_mode: str
...
@dataclass(frozen=True)
class _TrendBreadthResolvedParams:
...
@dataclass(frozen=True)
class _TrendBreadthSourceFrames:
    source_mode: SourceMode
    source_detail: str
    available_start_date: str | None
    available_end_date: str | None
...
@dataclass(frozen=True)
class _TrendBreadthCandidateFrames:
    candidate_metrics_df: pd.DataFrame
    candidate_comparison_df: pd.DataFrame
    selection_summary_df: pd.DataFrame
    rank_stability_df: pd.DataFrame
...
@dataclass(frozen=True)
class _TrendBreadthWalkforwardFrames:
...
def get_topix_downside_return_standard_deviation_trend_breadth_overlay_available_date_range(
    db_path: str,
...
def run_topix_downside_return_standard_deviation_trend_breadth_overlay_research(
    db_path: str,
    *,
    ...
    trend_rules: Sequence[str] | None = None,
    breadth_rules: Sequence[str] | None = None,
...
) -> TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult:
    params = _resolve_trend_breadth_params(
    ...
    source_frames = _build_trend_breadth_source_frames(
        db_path=db_path,
        start_date=start_date,
        end_date=end_date,
        params=params,
    )
    candidate_frames = _build_trend_breadth_candidate_frames(
        params=params,
    ...
    walkforward_frames = _build_trend_breadth_walkforward_frames(
        params=params,
        baseline_daily_df=source_frames.baseline_daily_df,
    ...
    return TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult(
        db_path=db_path,
        ...
        breadth_universe_membership_mode="latest_scale_category_proxy",
        ...
        trend_rules=params.trend_rules,
        breadth_rules=params.breadth_rules,
...
def _resolve_trend_breadth_params(
    *,
    ...
    trend_rules: Sequence[str] | None,
    breadth_rules: Sequence[str] | None,
    ...
    resolved_trend_rules = _normalize_rule_sequence(
        trend_rules,
        default=DEFAULT_TREND_RULES,
        name="trend_rules",
        valid_values=_VALID_TREND_RULES,
    )
    resolved_breadth_rules = _normalize_rule_sequence(
        breadth_rules,
        default=DEFAULT_BREADTH_RULES,
        name="breadth_rules",
    ...
    _validate_trend_breadth_params(
        validation_ratio=validation_ratio,
        min_constituents_per_day=min_constituents_per_day,
        discovery_window_days=discovery_window_days,
        validation_window_days=validation_window_days,
    ...
    parameter_grid = _build_trend_breadth_parameter_grid(
...
def _validate_trend_breadth_params(
    *,
    validation_ratio: float,
    min_constituents_per_day: int,
    discovery_window_days: int,
...
def _build_trend_breadth_parameter_grid(
    *,
    ...
    trend_rules: Sequence[str],
    breadth_rules: Sequence[str],
) -> list[dict[str, Any]]:
    parameter_grid = [
        {
        ...
        for std_window, mean_window, high_threshold, low_threshold, reduced_exposure_ratio, trend_rule, breadth_rule in product(
            std_windows,
            mean_windows,
            high_thresholds,
            low_thresholds,
...
def _build_trend_breadth_source_frames(
    *,
    db_path: str,
    start_date: str | None,
    end_date: str | None,
    params: _TrendBreadthResolvedParams,
) -> _TrendBreadthSourceFrames:
    with _open_analysis_connection(db_path) as ctx:
    ...
    breadth_history_df = _filter_trend_breadth_history(
        breadth_history_df,
        start_date=start_date,
        end_date=end_date,
    )
    ...
    market_frame_df = _build_topix_trend_feature_df(
        _prepare_topix_market_frame(topix_daily_df)
    )
    ...
    return _TrendBreadthSourceFrames(
...
def _filter_trend_breadth_history(
...
def _build_trend_breadth_candidate_frames(
    *,
    params: _TrendBreadthResolvedParams,
    market_frame_df: pd.DataFrame,
    breadth_daily_df: pd.DataFrame,
    ...
    for parameter_row in params.parameter_grid:
        candidate_daily_df = _simulate_trend_breadth_candidate(
        ...
        candidate_return_series_by_id[candidate_id] = candidate_daily_df[
            "strategy_return"
        ].astype(float)
        candidate_key = _build_trend_breadth_candidate_selection_key(
            candidate_daily_df,
    ...
    candidate_metrics_df = _build_trend_breadth_candidate_metrics_df(candidate_metric_rows)
    ...
    return _TrendBreadthCandidateFrames(
...
def _simulate_trend_breadth_candidate(
    *,
    ...
    return _simulate_candidate_daily_df_with_regime_filters(
        candidate_id=str(parameter_row["candidate_id"]),
        ...
        trend_rule=str(parameter_row["trend_rule"]),
        breadth_rule=str(parameter_row["breadth_rule"]),
    )
...
def _build_trend_breadth_candidate_selection_key(
    candidate_daily_df: pd.DataFrame,
    *,
    candidate_id: str,
...
def _build_trend_breadth_candidate_metrics_df(
    candidate_metric_rows: list[dict[str, Any]],
) -> pd.DataFrame:
...
def _build_trend_breadth_walkforward_frames(
    *,
    params: _TrendBreadthResolvedParams,
    baseline_daily_df: pd.DataFrame,
    candidate_comparison_df: pd.DataFrame,
    candidate_return_series_by_id: dict[str, pd.Series],
) -> _TrendBreadthWalkforwardFrames:
    (
        walkforward_fold_candidate_rank_df,
        walkforward_rank_diagnostics_df,
        walkforward_top1_df,
    ...
    return _TrendBreadthWalkforwardFrames(
...
def write_topix_downside_return_standard_deviation_trend_breadth_overlay_research_bundle(
    result: TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult,
    *,
    ...
    return write_dataclass_research_bundle(
        experiment_id=TOPIX_DOWNSIDE_RETURN_STANDARD_DEVIATION_TREND_BREADTH_OVERLAY_EXPERIMENT_ID,
        module=__name__,
        function="run_topix_downside_return_standard_deviation_trend_breadth_overlay_research",
        params={
            ...
            "trend_rules": list(result.trend_rules),
            "breadth_rules": list(result.breadth_rules),
...
def load_topix_downside_return_standard_deviation_trend_breadth_overlay_research_bundle(
    bundle_path: str | Path,
) -> TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult:
    return load_dataclass_research_bundle(
        bundle_path,
        result_type=TopixDownsideReturnStandardDeviationTrendBreadthOverlayResearchResult,
        table_field_names=_RESULT_TABLE_NAMES,
    )


def get_topix_downside_return_standard_deviation_trend_breadth_overlay_latest_bundle_path(
    *,
    output_root: str | Path | None = None,
) -> Path | None:
...
def get_topix_downside_return_standard_deviation_trend_breadth_overlay_bundle_path_for_run_id(
    run_id: str,
    *,
    output_root: str | Path | None = None,
) -> Path:
...
def _build_topix_trend_feature_df(market_frame_df: pd.DataFrame) -> pd.DataFrame:
    feature_df = market_frame_df.copy()
    feature_df["topix_sma20"] = feature_df["close"].rolling(20, min_periods=20).mean()
    feature_df["topix_sma60"] = feature_df["close"].rolling(60, min_periods=60).mean()
    feature_df["topix_return_10d"] = feature_df["close"].div(feature_df["close"].shift(10)).sub(1.0)
    feature_df["topix_drawdown_63d"] = feature_df["close"].div(
        feature_df["close"].rolling(63, min_periods=63).max()
    ).sub(1.0)
    feature_df["topix_close_below_sma20"] = feature_df["close"] < feature_df["topix_sma20"]
    feature_df["topix_sma20_below_sma60"] = feature_df["topix_sma20"] < feature_df["topix_sma60"]
    return feature_df


def _build_topix100_breadth_daily_df(
    history_df: pd.DataFrame,
    *,
    min_constituents_per_day: int,
) -> pd.DataFrame:
    panel_df = history_df[["code", "date", "close"]].copy()
    panel_df["date"] = panel_df["date"].astype(str)
    panel_df = panel_df.sort_values(["code", "date"]).reset_index(drop=True)

    grouped_close = panel_df.groupby("code", group_keys=False)["close"]
    panel_df["sma20"] = grouped_close.rolling(20, min_periods=20).mean().reset_index(
        level=0,
        drop=True,
    )
    panel_df["return_5d"] = grouped_close.pct_change(periods=5)
    panel_df["rolling_low_20"] = grouped_close.rolling(20, min_periods=20).min().reset_index(
        level=0,
        drop=True,
    )
    panel_df["breadth_above_sma20_flag"] = (
        panel_df["close"] > panel_df["sma20"]
    ).where(panel_df["sma20"].notna())
    panel_df["breadth_positive_5d_flag"] = (
        panel_df["return_5d"] > 0.0
    ).where(panel_df["return_5d"].notna())
    panel_df["breadth_at_20d_low_flag"] = (
        panel_df["close"] <= panel_df["rolling_low_20"]
    ).where(panel_df["rolling_low_20"].notna())

    breadth_daily_df = (
        panel_df.groupby("date", as_index=False)
        .agg(
            stock_count=("code", "nunique"),
            breadth_above_sma20_ratio=("breadth_above_sma20_flag", "mean"),
            breadth_positive_5d_ratio=("breadth_positive_5d_flag", "mean"),
            breadth_at_20d_low_ratio=("breadth_at_20d_low_flag", "mean"),
            equal_weight_return_5d=("return_5d", "mean"),
        )
        .sort_values("date", ignore_index=True)
    )
    breadth_daily_df = breadth_daily_df[
        breadth_daily_df["stock_count"] >= min_constituents_per_day
    ].copy()
    if breadth_daily_df.empty:
        raise ValueError("No breadth dates survived the minimum constituent filter")
    return breadth_daily_df


def _build_common_signal_frame_with_regimes(
    market_frame_df: pd.DataFrame,
    *,
    breadth_daily_df: pd.DataFrame,
    ...
    signal_df = signal_df.drop(columns=["date"])
    ...
    signal_df = signal_df.dropna(
        subset=[
            ...
            "topix_return_10d",
            "topix_drawdown_63d",
            "breadth_above_sma20_ratio",
            "breadth_positive_5d_ratio",
            "breadth_at_20d_low_ratio",
        ]
    ).reset_index(drop=True)
    if signal_df.empty:
        raise ValueError("No signal rows remained after trend and breadth warmup")
    ...
    return signal_df[
        [
            ...
            "topix_return_10d",
            "topix_drawdown_63d",
            "topix_close_below_sma20",
            "topix_sma20_below_sma60",
            "stock_count",
            "breadth_above_sma20_ratio",
            "breadth_positive_5d_ratio",
            "breadth_at_20d_low_ratio",
...
def _build_candidate_signal_frame(
    ...
    *,
    ...
    signal_base_df = _build_common_signal_frame_with_regimes(
        market_frame_df,
        breadth_daily_df=breadth_daily_df,
        max_downside_return_standard_deviation_window_days=stddev_window_days,
        max_downside_return_standard_deviation_mean_window_days=mean_window_days,
        validation_ratio=validation_ratio,
    )
    ...
    candidate_signal_df = candidate_signal_df.dropna(
        subset=["annualized_downside_return_standard_deviation_mean"]
    ).reset_index(drop=True)
    if candidate_signal_df.empty:
        raise ValueError(
            "Common signal frame reached rows without enough downside return-standard-deviation history"
        )
    candidate_signal_df["sample_split"] = _build_sample_split_labels(
...
def _simulate_candidate_daily_df_with_regime_filters(
    *,
    candidate_id: str,
    ...
    for row in candidate_signal_df.itertuples(index=False):
        ...
        trend_triggered = _evaluate_trend_rule(row, trend_rule=trend_rule)
        breadth_triggered = _evaluate_breadth_rule(row, breadth_rule=breadth_rule)
        ...
        if indicator_value < low_annualized_downside_return_standard_deviation_threshold:
            ...
            and trend_triggered
            and breadth_triggered
        ):
            next_exposure_ratio = reduced_exposure_ratio
            signal_state = "risk_off_confirmed"
        elif indicator_value > high_annualized_downside_return_standard_deviation_threshold:
            signal_state = "high_threshold_unconfirmed"
...
def _build_walkforward_top1_outputs(
    *,
    ...
    candidate_return_panel_df = pd.concat(
        [
            pd.DataFrame(
                {"realized_date": baseline_daily_df["realized_date"].reset_index(drop=True)}
            ),
            pd.DataFrame(
                {
                    candidate_id: candidate_return_series_by_id[candidate_id].reset_index(
                        drop=True
                    )
...
def _build_research_bundle_summary_markdown(
    ...
    lines = [
        "# TOPIX Downside Return Standard Deviation x Trend x Breadth Overlay",
        "",
        "## Scope",
        "",
        "This bundle keeps the long-only TOPIX overlay baseline, but only allows risk-off transitions when downside return standard deviation expands and the move is confirmed by weak TOPIX trend plus weak TOPIX100 breadth.",
        "",
        "## Search Grid",
        "",
        ...
        f"- Trend rules: `{', '.join(result.trend_rules)}`",
        f"- Breadth rules: `{', '.join(result.breadth_rules)}`",
        f"- Minimum TOPIX100 constituents per day: `{result.min_constituents_per_day}`",
        f"- Breadth universe: `{result.breadth_universe}` (`{result.breadth_universe_membership_mode}`)",
    ...
    if best_sharpe_row is not None:
        lines.extend(
            [
                ...
                f"- Params: `std={int(best_sharpe_row['downside_return_standard_deviation_window_days'])}`, `mean={int(best_sharpe_row['downside_return_standard_deviation_mean_window_days'])}`, `high={float(best_sharpe_row['high_annualized_downside_return_standard_deviation_threshold']):.2f}`, `low={float(best_sharpe_row['low_annualized_downside_return_standard_deviation_threshold']):.2f}`, `reduced={float(best_sharpe_row['reduced_exposure_ratio']):.2f}`, `trend={best_sharpe_row['trend_rule']}`, `breadth={best_sharpe_row['breadth_rule']}`",
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- The overlay stays long-only and only reduces exposure; it never exceeds 100% exposure.",
            "- Risk-off entry requires a three-part confirmation: downside return standard deviation, weak TOPIX trend, and weak TOPIX100 breadth.",
            "- The breadth universe currently uses the latest scale-category snapshot as a practical TOPIX100 proxy because historical constituent snapshots are not stored in market.duckdb.",
        ]
    )
    return "\n".join(lines)
...
def _build_candidate_id(
    *,
    ...
    return (
        ...
        f"trend_{trend_rule}_breadth_{breadth_rule}"
    )
...
def _normalize_rule_sequence(
    ...
    *,
    ...
    raw_values = tuple(default if values is None else tuple(str(value).strip() for value in values))
    if not raw_values:
        raise ValueError(f"{name} must not be empty")
    if any(not value for value in raw_values):
        raise ValueError(f"{name} must not contain blank values")
    invalid_values = sorted(set(raw_values) - valid_values)
    if invalid_values:
        raise ValueError(f"{name} contains unsupported values: {', '.join(invalid_values)}")

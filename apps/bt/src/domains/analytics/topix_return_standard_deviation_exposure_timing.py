"""
TOPIX return-standard-deviation exposure timing research.

This study asks whether scaling a TOPIX long book down when return standard
deviation expands can improve risk-adjusted performance relative to a constant
100% long baseline.
...
DEFAULT_RETURN_STANDARD_DEVIATION_MEAN_WINDOW_DAYS: tuple[int, ...] = (1, 3, 5)
DEFAULT_HIGH_ANNUALIZED_RETURN_STANDARD_DEVIATION_THRESHOLDS: tuple[float, ...] = (
    0.15,
    0.20,
    0.25,
    0.30,
)
DEFAULT_LOW_ANNUALIZED_RETURN_STANDARD_DEVIATION_THRESHOLDS: tuple[float, ...] = (
    0.10,
    0.15,
    0.20,
    0.25,
...
TOPIX_RETURN_STANDARD_DEVIATION_EXPOSURE_TIMING_EXPERIMENT_ID = (
    "market-behavior/topix-return-standard-deviation-exposure-timing"
)
...
class TopixReturnStandardDeviationExposureTimingResearchResult:
    db_path: str
    ...
    return_standard_deviation_window_days: tuple[int, ...]
    return_standard_deviation_mean_window_days: tuple[int, ...]
    high_annualized_return_standard_deviation_thresholds: tuple[float, ...]
    low_annualized_return_standard_deviation_thresholds: tuple[float, ...]
...
def get_topix_return_standard_deviation_exposure_timing_available_date_range(
    db_path: str,
...
def run_topix_return_standard_deviation_exposure_timing_research(
    db_path: str,
    *,
    start_date: str | None = None,
    end_date: str | None = None,
    return_standard_deviation_window_days: Sequence[int] | None = None,
    return_standard_deviation_mean_window_days: Sequence[int] | None = None,
    high_annualized_return_standard_deviation_thresholds: Sequence[float] | None = None,
    low_annualized_return_standard_deviation_thresholds: Sequence[float] | None = None,
    reduced_exposure_ratios: Sequence[float] | None = None,
    validation_ratio: float = DEFAULT_VALIDATION_RATIO,
) -> TopixReturnStandardDeviationExposureTimingResearchResult:
    resolved_stddev_windows = _normalize_positive_int_sequence(
        return_standard_deviation_window_days,
        default=DEFAULT_RETURN_STANDARD_DEVIATION_WINDOW_DAYS,
        name="return_standard_deviation_window_days",
    )
    resolved_mean_windows = _normalize_positive_int_sequence(
        return_standard_deviation_mean_window_days,
        default=DEFAULT_RETURN_STANDARD_DEVIATION_MEAN_WINDOW_DAYS,
        name="return_standard_deviation_mean_window_days",
    )
    resolved_high_thresholds = _normalize_non_negative_float_sequence(
        high_annualized_return_standard_deviation_thresholds,
        default=DEFAULT_HIGH_ANNUALIZED_RETURN_STANDARD_DEVIATION_THRESHOLDS,
        name="high_annualized_return_standard_deviation_thresholds",
    )
    resolved_low_thresholds = _normalize_non_negative_float_sequence(
        low_annualized_return_standard_deviation_thresholds,
        default=DEFAULT_LOW_ANNUALIZED_RETURN_STANDARD_DEVIATION_THRESHOLDS,
        name="low_annualized_return_standard_deviation_thresholds",
    )
    ...
    signal_base_df = _build_common_signal_frame(
        market_frame_df,
        max_return_standard_deviation_window_days=max(resolved_stddev_windows),
        max_return_standard_deviation_mean_window_days=max(resolved_mean_windows),
    ...
    for (
        ...
        if signal_key not in signal_frame_cache:
            signal_frame_cache[signal_key] = _build_candidate_signal_frame(
                ...
                return_standard_deviation_window_days=stddev_window_days,
                return_standard_deviation_mean_window_days=mean_window_days,
            )
        ...
        candidate_daily_df = _simulate_candidate_daily_df(
            ...
            high_annualized_return_standard_deviation_threshold=high_threshold,
            low_annualized_return_standard_deviation_threshold=low_threshold,
            reduced_exposure_ratio=reduced_exposure_ratio,
        )
    ...
    return TopixReturnStandardDeviationExposureTimingResearchResult(
        db_path=db_path,
        ...
        return_standard_deviation_window_days=resolved_stddev_windows,
        return_standard_deviation_mean_window_days=resolved_mean_windows,
        high_annualized_return_standard_deviation_thresholds=resolved_high_thresholds,
        low_annualized_return_standard_deviation_thresholds=resolved_low_thresholds,
...
def write_topix_return_standard_deviation_exposure_timing_research_bundle(
    result: TopixReturnStandardDeviationExposureTimingResearchResult,
    *,
    ...
    return write_dataclass_research_bundle(
        experiment_id=TOPIX_RETURN_STANDARD_DEVIATION_EXPOSURE_TIMING_EXPERIMENT_ID,
        module=__name__,
        function="run_topix_return_standard_deviation_exposure_timing_research",
        params={
            "start_date": result.analysis_start_date,
            "end_date": result.analysis_end_date,
            "validation_ratio": result.validation_ratio,
            "return_standard_deviation_window_days": list(result.return_standard_deviation_window_days),
            "return_standard_deviation_mean_window_days": list(result.return_standard_deviation_mean_window_days),
            "high_annualized_return_standard_deviation_thresholds": list(
                result.high_annualized_return_standard_deviation_thresholds
            ),
            "low_annualized_return_standard_deviation_thresholds": list(
                result.low_annualized_return_standard_deviation_thresholds
            ),
...
def load_topix_return_standard_deviation_exposure_timing_research_bundle(
    bundle_path: str | Path,
) -> TopixReturnStandardDeviationExposureTimingResearchResult:
    return load_dataclass_research_bundle(
        bundle_path,
        result_type=TopixReturnStandardDeviationExposureTimingResearchResult,
        table_field_names=_RESULT_TABLE_NAMES,
    )


def get_topix_return_standard_deviation_exposure_timing_latest_bundle_path(
    *,
    output_root: str | Path | None = None,
) -> Path | None:
...
def get_topix_return_standard_deviation_exposure_timing_bundle_path_for_run_id(
    run_id: str,
    *,
    output_root: str | Path | None = None,
) -> Path:
...
def _build_common_signal_frame(
    ...
    *,
    max_return_standard_deviation_window_days: int,
    max_return_standard_deviation_mean_window_days: int,
    validation_ratio: float,
) -> pd.DataFrame:
    signal_start_index = (
        max_return_standard_deviation_window_days
        + max_return_standard_deviation_mean_window_days
        - 1
    )
    signal_stop_index = len(market_frame_df) - 1
    if signal_start_index >= signal_stop_index:
        raise ValueError(
            "Not enough TOPIX rows for the requested return standard deviation windows: "
...
def _build_candidate_signal_frame(
    ...
    *,
    ...
    return_standard_deviation_window_days: int,
    return_standard_deviation_mean_window_days: int,
) -> pd.DataFrame:
    indicator_df = market_frame_df[["date", "close_return"]].copy()
    indicator_df["return_standard_deviation"] = indicator_df["close_return"].rolling(
        return_standard_deviation_window_days,
        min_periods=return_standard_deviation_window_days,
    ).std(ddof=1)
    indicator_df["annualized_return_standard_deviation"] = (
        indicator_df["return_standard_deviation"] * math.sqrt(ANNUALIZATION_FACTOR)
    )
    indicator_df["annualized_return_standard_deviation_mean"] = indicator_df[
        "annualized_return_standard_deviation"
    ].rolling(
        return_standard_deviation_mean_window_days,
        min_periods=return_standard_deviation_mean_window_days,
    ).mean()
    ...
    candidate_signal_df = signal_base_df.merge(
        indicator_df[
            [
                ...
                "annualized_return_standard_deviation",
                "annualized_return_standard_deviation_mean",
            ]
        ],
        left_on="signal_date",
        right_on="date",
    ...
    if candidate_signal_df["annualized_return_standard_deviation_mean"].isna().any():
        raise ValueError(
            "Common signal frame reached rows without enough return-standard-deviation history"
        )
...
def _simulate_candidate_daily_df(
    *,
    ...
    high_annualized_return_standard_deviation_threshold: float,
    low_annualized_return_standard_deviation_threshold: float,
    ...
    for row in candidate_signal_df.itertuples(index=False):
        indicator_value = _coerce_numeric_value(row.annualized_return_standard_deviation_mean)
        ...
        if indicator_value > high_annualized_return_standard_deviation_threshold:
            next_exposure_ratio = reduced_exposure_ratio
            signal_state = "above_high_threshold"
        elif indicator_value < low_annualized_return_standard_deviation_threshold:
        ...
        rows.append(
            {
                ...
                "signal_return_standard_deviation": _coerce_numeric_value(
                    row.return_standard_deviation
                ),
                "signal_annualized_return_standard_deviation": _coerce_numeric_value(
                    row.annualized_return_standard_deviation
                ),
                "signal_annualized_return_standard_deviation_mean": indicator_value,
    ...
    comparison_df = (
        candidate_metrics_df[
            [
                ...
                "return_standard_deviation_window_days",
                "return_standard_deviation_mean_window_days",
                "high_annualized_return_standard_deviation_threshold",
                "low_annualized_return_standard_deviation_threshold",
                "reduced_exposure_ratio",
                "sample_split",
                *value_columns,
            ]
...
def _build_research_bundle_summary_markdown(
    ...
    lines = [
        ...
        "## Scope",
        "",
        "This bundle tests whether a simple TOPIX long book can improve Sharpe or CAGR by reducing exposure when return standard deviation expands and restoring full exposure when return standard deviation compresses again.",
        "",
        "Execution convention:",
        "",
        ...
        "## Search Grid",
        "",
        f"- Return standard deviation windows: `{_format_int_sequence(result.return_standard_deviation_window_days)}`",
        f"- Return standard deviation mean windows: `{_format_int_sequence(result.return_standard_deviation_mean_window_days)}`",
        f"- High annualized return standard deviation thresholds: `{_format_float_sequence(result.high_annualized_return_standard_deviation_thresholds)}`",
        f"- Low annualized return standard deviation thresholds: `{_format_float_sequence(result.low_annualized_return_standard_deviation_thresholds)}`",
    ...
    if best_sharpe_row is not None:
        lines.extend(
            [
                ...
                f"- Params: `std={int(best_sharpe_row['return_standard_deviation_window_days'])}`, `mean={int(best_sharpe_row['return_standard_deviation_mean_window_days'])}`, `high={float(best_sharpe_row['high_annualized_return_standard_deviation_threshold']):.2f}`, `low={float(best_sharpe_row['low_annualized_return_standard_deviation_threshold']):.2f}`, `reduced={float(best_sharpe_row['reduced_exposure_ratio']):.2f}`",
    ...
    lines.extend(
        [
            ...
            "- Thresholds are applied on annualized return standard deviation so the grid remains interpretable.",

...
class BacktestExecutorMixin:
    ...
    def _create_round_trip_portfolio(
        ...
        *,
        ...
        return self._get_execution_adapter().create_round_trip_portfolio(
            open_data=open_data,
    ...
    def _create_grouped_portfolio(
        ...
        adapter = self._get_execution_adapter()
        ...
        if len(self.stock_codes) > 1:
            ...
            return adapter.create_signal_portfolio(
                close=close_data,
                entries=all_entries,
                exits=all_exits,
                direction=getattr(self, "direction", "longonly"),
                init_cash=self.initial_cash,
                size=allocation_per_asset,
                size_type="percent",
                fees=effective_fees,
                slippage=effective_slippage,
                cash_sharing=True,
                group_by=True,
                accumulate=pyramid_enabled,
                call_seq="auto",
                max_size=self.max_exposure,
                freq="D",
            )
    ...
    def _create_individual_portfolios(
        ...
        Returns:
            ExecutionPortfolioProtocol: 個別ポートフォリオ
        """
        ...
        if self._uses_round_trip_execution():
            ...
            effective_fees, effective_slippage = self._calculate_cost_params()
            portfolio = adapter.create_signal_portfolio(
                close=close_data,
    ...
    def run_optimized_backtest(
        ...
        Returns:
            Tuple[ExecutionPortfolioProtocol, ExecutionPortfolioProtocol, AllocationInfo]:
                - 第1段階結果
                - 第2段階最適化結果
                - Kelly基準統計情報
        """

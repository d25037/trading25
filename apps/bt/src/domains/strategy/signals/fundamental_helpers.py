...
def _to_float_series(data: NumericSeries) -> pd.Series[float]:
    """Coerce numeric-like Series inputs to float for arithmetic/comparison."""
    return pd.to_numeric(data, errors="coerce").astype(float)
...
def _calc_growth_signal(
    ...
    """
    data_float = _to_float_series(data)

    if periods <= 0:
        # periods=0は現在値との比較（常に成長率0%）
        return pd.Series(False, index=data_float.index)

    # 値の変化点を検出（=決算発表日）、最初の値も発表日とみなす
    is_release_date = data_float.diff() != 0
    ...
    release_values = data_float.loc[release_dates]
    ...
    daily_result = pd.Series(np.nan, index=data_float.index)
    daily_result.loc[meets_condition.index] = meets_condition.astype(float)

    return daily_result.ffill().fillna(0.0).astype(bool) & data_float.notna()
...
def _calc_threshold_signal(
    ...
    if condition == "above":
        base_condition = (data_float >= threshold) & data_float.notna()
    else:
        base_condition = (data_float < threshold) & data_float.notna()
...
def _calc_ratio_signal(
    ...
    Returns:
        pd.Series[bool]: 条件を満たす場合True
    """
    ratio_float = _to_float_series(ratio)
    valid_ratio = (
        ratio_float.notna() & (ratio_float > 0)
        if exclude_negative
        else ratio_float.notna()
    )
    threshold_condition = (
...
def _calc_consecutive_threshold_signal(
    ...
    # 閾値条件の判定
    meets_threshold = (
        data_float >= threshold if condition == "above" else data_float < threshold
    )

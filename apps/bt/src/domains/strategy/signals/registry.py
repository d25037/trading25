...
SIGNAL_REGISTRY: list[SignalDefinition] = [
    ...
    SignalDefinition(
        name="CMF閾値",
        signal_func=cmf_threshold_signal,
        enabled_checker=lambda p: p.cmf_threshold.enabled,
        param_builder=lambda p, d: {
            "high": d["ohlc_data"]["High"],
            "low": d["ohlc_data"]["Low"],
            "close": d["ohlc_data"]["Close"],
            "volume": d["volume"],
            "period": p.cmf_threshold.period,
            "threshold": p.cmf_threshold.threshold,
            "condition": p.cmf_threshold.condition,
        },
        entry_purpose="価格が走る前の出来高付き買い集め候補をCMFで絞り込む",
        exit_purpose="CMF悪化を検出",
        category="volume",
        description="Chaikin Money Flowが閾値以上/未満かを判定",
        param_key="cmf_threshold",
        data_checker=lambda d: "ohlc_data" in d and "volume" in d,
        data_requirements=["ohlc", "volume"],
    ),
    # 2-4. Chaikin oscillatorシグナル
    SignalDefinition(
        name="Chaikin oscillator",
        signal_func=chaikin_oscillator_signal,
        enabled_checker=lambda p: p.chaikin_oscillator.enabled,
        param_builder=lambda p, d: {
            "high": d["ohlc_data"]["High"],
            "low": d["ohlc_data"]["Low"],
            "close": d["ohlc_data"]["Close"],
            "volume": d["volume"],
            "fast_period": p.chaikin_oscillator.fast_period,
            "slow_period": p.chaikin_oscillator.slow_period,
            "threshold": p.chaikin_oscillator.threshold,
            "condition": p.chaikin_oscillator.condition,
        },
        entry_purpose="ADLの短期加速が強い銘柄を絞り込む",
        exit_purpose="ADL加速の悪化を検出",
        category="volume",
        description="Chaikin Oscillator（ADL短期EMA - ADL長期EMA）の閾値条件を判定",
        param_key="chaikin_oscillator",
        data_checker=lambda d: "ohlc_data" in d and "volume" in d,
        data_requirements=["ohlc", "volume"],
    ),
    # 2-5. OBV flow scoreシグナル
    SignalDefinition(
        name="OBV flow score",
        signal_func=obv_flow_score_signal,
        enabled_checker=lambda p: p.obv_flow_score.enabled,
        param_builder=lambda p, d: {
            "close": d["ohlc_data"]["Close"],
            "volume": d["volume"],
            "lookback_period": p.obv_flow_score.lookback_period,
            "threshold": p.obv_flow_score.threshold,
            "condition": p.obv_flow_score.condition,
        },
        entry_purpose="OBVの直近蓄積フローが強い銘柄を絞り込む",
        exit_purpose="OBVフロー悪化を検出",
        category="volume",
        description="On-Balance Volumeのlookback変化を出来高で正規化して判定",
        param_key="obv_flow_score",
        data_checker=lambda d: "ohlc_data" in d and "volume" in d,
        data_requirements=["ohlc", "volume"],
    ),
    # 2-6. Accumulation pressure複合シグナル
    SignalDefinition(
        name="Accumulation pressure",
        signal_func=accumulation_pressure_signal,
        enabled_checker=lambda p: p.accumulation_pressure.enabled,
        param_builder=lambda p, d: {
            "high": d["ohlc_data"]["High"],
            "low": d["ohlc_data"]["Low"],
            "close": d["ohlc_data"]["Close"],
            "volume": d["volume"],
            "cmf_period": p.accumulation_pressure.cmf_period,
            "chaikin_fast_period": p.accumulation_pressure.chaikin_fast_period,
            "chaikin_slow_period": p.accumulation_pressure.chaikin_slow_period,
            "obv_lookback_period": p.accumulation_pressure.obv_lookback_period,
            "cmf_threshold": p.accumulation_pressure.cmf_threshold,
            "chaikin_oscillator_threshold": p.accumulation_pressure.chaikin_oscillator_threshold,
            "obv_score_threshold": p.accumulation_pressure.obv_score_threshold,
            "min_votes": p.accumulation_pressure.min_votes,
        },
        entry_purpose="CMF/Chaikin/OBVの複数成立で買い集めproxyを絞り込む",
        exit_purpose="買い集めproxyの消失を検出",
        category="volume",
        description="CMF、Chaikin oscillator、OBV flow scoreの投票式蓄積圧力判定",
        param_key="accumulation_pressure",
        data_checker=lambda d: "ohlc_data" in d and "volume" in d,
        data_requirements=["ohlc", "volume"],
    ),
    # 3. PERシグナル
    ...
    SignalDefinition(
        ...
        category="macro",
        description="設定ユニバース内で close / SMA(period) を順位化し、q1 / q10 / q456 / other を判定",

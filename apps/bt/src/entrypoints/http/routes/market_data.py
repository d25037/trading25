...
from src.application.contracts import market_data as market_data_contracts
...
@router.get(
    "/api/market/stocks",
    response_model=list[market_data_contracts.MarketStockData],
...
) -> list[market_data_contracts.MarketStockData]:
...
@router.get(
    ...
    response_model=market_data_contracts.StockInfo,
    summary="単一銘柄情報取得",
)
def get_stock_info(
    request: Request,
...
) -> market_data_contracts.StockInfo:
    service = _get_market_data_service(request)
...
@router.get(
    ...
    response_model=list[market_data_contracts.MarketOHLCVRecord],
...
) -> list[market_data_contracts.MarketOHLCVRecord]:
    service = _get_market_data_service(request)
...
@router.get(
    ...
    response_model=list[market_data_contracts.MarketMinuteBarRecord],
    summary="銘柄 分足データ取得",
)
...
) -> list[market_data_contracts.MarketMinuteBarRecord]:
...
@router.get(
    "/api/market/topix",
    response_model=list[market_data_contracts.MarketOHLCRecord],
    summary="TOPIX データ取得",
)
def get_topix(
    request: Request,
    start_date: str | None = Query(default=None, description="開始日 (YYYY-MM-DD)"),
    end_date: str | None = Query(default=None, description="終了日 (YYYY-MM-DD)"),
) -> list[market_data_contracts.MarketOHLCRecord]:
    service = _get_market_data_service(request)
...
@router.get(
    ...
    response_model=jquants_contracts.N225OptionsExplorerResponse,

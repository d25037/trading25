...
class ConfigLoader:
    ...
    def save_strategy_config(
        ...
        # config_dirがデフォルト(config)の場合のみ外部ディレクトリを使用
        # テスト時はtmp_pathが渡されるため、config_dir内に保存
        is_default_config = str(self.config_dir) == "config"

        # 保存先パスを決定（experimentalは外部ディレクトリ、ただしテスト時はconfig_dir内）
    ...
    def duplicate_strategy(
        self, source_strategy: str, new_strategy_name: str
    ) -> Path:
        """
        既存戦略を複製して新しい戦略を作成
        ...
        """
        from src.paths import get_strategies_dir, find_strategy_path
        ...
        if is_default_config:
            target_path = get_strategies_dir("experimental") / f"{new_strategy_name}.yaml"
            # 既に存在する場合はエラー（外部・プロジェクト両方チェック）
            ...
            # テストモード: config_dir内のみチェック
            target_path = self.config_dir / "strategies" / "experimental" / f"{new_strategy_name}.yaml"
    ...
    def rename_strategy(self, strategy_name: str, new_name: str) -> Path:
        ...
        if is_default_config:
            ...
            # テストモード: config_dir内のみチェック
            new_path = self.config_dir / "strategies" / "experimental" / f"{new_name}.yaml"
            if new_path.exists():
                raise FileExistsError(f"戦略 '{new_name}' は既に存在します")

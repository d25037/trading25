...
class TestSignalService:
    ...
    def test_load_signal_data_ignores_all_placeholder_for_universe_signal(
        self,
        service: SignalService,
    ) -> None:
        service._market_reader = object()  # type: ignore[assignment]
        ...
        with (
            ...
            patch(
                "src.application.services.signal_service.load_market_multi_data",
                return_value=({"7203": {"daily": daily}}, []),
            ) as load_multi_data,
        ):
            loaded = service._load_signal_data(
                "7203",
                shared_config=_make_shared_config(stock_codes=["all"]),
                entry_params=entry_params,
                exit_params=SignalParams(),
                start_date=date(2025, 1, 1),
                end_date=date(2025, 1, 5),
        ...
        assert build_requirements.call_args.kwargs["stock_codes"] == ("7203",)
        assert load_multi_data.call_args.args[1] == ["7203"]
        assert loaded.universe_member_codes == ["7203"]
        assert any(
            "explicit shared_config.stock_codes" in warning

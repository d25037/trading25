...
def test_vi_regime_conditioning_bundle_roundtrip(
    analytics_db_path: str,
    tmp_path: Path,
) -> None:
    result = run_topix100_vi_change_regime_conditioning_research(
    ...
    reloaded = load_topix100_vi_change_regime_conditioning_research_bundle(
        bundle.bundle_dir
    )

    assert bundle.experiment_id == TOPIX100_VI_CHANGE_REGIME_RESEARCH_EXPERIMENT_ID
    assert (
        get_topix100_vi_change_regime_conditioning_bundle_path_for_run_id(
            bundle.run_id,
            output_root=tmp_path,

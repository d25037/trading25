...
from src.domains.analytics.topix_downside_return_standard_deviation_exposure_timing import (
    ...
    get_topix_downside_return_standard_deviation_exposure_timing_bundle_path_for_run_id,
    get_topix_downside_return_standard_deviation_exposure_timing_latest_bundle_path,
    load_topix_downside_return_standard_deviation_exposure_timing_research_bundle,
...
def test_build_candidate_signal_frame_uses_negative_returns_only() -> None:
    market_frame_df = pd.DataFrame(
        {
            "date": ["2024-01-01", "2024-01-02", "2024-01-03", "2024-01-04"],
            "close_return": [0.01, -0.02, 0.03, -0.04],
        }
    )
    signal_base_df = pd.DataFrame(
        {
            ...
            "realized_close_return": [0.0, 0.0, 0.0],
            "realized_overnight_return": [0.0, 0.0, 0.0],
            "realized_intraday_return": [0.0, 0.0, 0.0],
        }
    )

    candidate_signal_df = _build_candidate_signal_frame(
        market_frame_df,
        signal_base_df=signal_base_df,
        downside_return_standard_deviation_window_days=2,
        downside_return_standard_deviation_mean_window_days=1,
    )

    expected_downside = [math.sqrt(0.0002), math.sqrt(0.0002), math.sqrt(0.0008)]
    assert candidate_signal_df["downside_return_standard_deviation"].tolist() == pytest.approx(
        expected_downside
    )
    assert candidate_signal_df[
        "annualized_downside_return_standard_deviation"
    ].tolist() == pytest.approx(
        [value * math.sqrt(ANNUALIZATION_FACTOR) for value in expected_downside]
    )
    assert candidate_signal_df[
...
def test_simulation_uses_previous_exposure_for_overnight_and_new_exposure_for_intraday() -> None:
    signal_df = pd.DataFrame(
        {
            ...
            "realized_date": ["2024-01-03", "2024-01-04", "2024-01-05"],
            "sample_split": ["discovery", "discovery", "validation"],
            "downside_return_standard_deviation": [0.010, 0.011, 0.009],
            "annualized_downside_return_standard_deviation": [0.20, 0.21, 0.19],
            "annualized_downside_return_standard_deviation_mean": [0.30, 0.22, 0.18],
    ...
    result_df = _simulate_candidate_daily_df(
        ...
        high_annualized_downside_return_standard_deviation_threshold=0.25,
        low_annualized_downside_return_standard_deviation_threshold=0.20,
...
def test_research_bundle_roundtrip_preserves_topix_downside_return_standard_deviation_result(
    tmp_path: Path,
) -> None:
    ...
    db_path = _build_topix_only_db(
        tmp_path / "market-topix-downside-return-standard-deviation.duckdb",
    ...
    result = run_topix_downside_return_standard_deviation_exposure_timing_research(
        ...
        downside_return_standard_deviation_mean_window_days=(2,),
    ...
    reloaded = load_topix_downside_return_standard_deviation_exposure_timing_research_bundle(
        bundle.bundle_dir
    )
    ...
    assert reloaded.downside_return_standard_deviation_window_days == (3,)
    assert reloaded.downside_return_standard_deviation_mean_window_days == (2,)

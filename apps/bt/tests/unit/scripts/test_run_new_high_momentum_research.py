"""Tests for the new-high momentum research runner script."""
...
def test_parse_args_accepts_new_high_momentum_options() -> None:
    ...
    assert args.high_windows == (20, 252)
    assert args.horizons == (5, 20)
    assert args.sample_event_size == 3
...
def test_main_runs_new_high_momentum_research_and_prints_bundle_payload(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    fake_bundle = SimpleNamespace(
        experiment_id="market-behavior/new-high-momentum-research",
...
def test_main_module_executes_as_script(monkeypatch) -> None:
    ...
    import src.domains.analytics.new_high_momentum_research as research_module

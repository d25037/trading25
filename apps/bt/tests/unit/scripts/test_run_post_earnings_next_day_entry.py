...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_post_earnings_next_day_entry.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_post_earnings_next_day_entry",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load post-earnings next-day runner module")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_post_earnings_next_day_entry"] = module

"""Tests for the speculative prime pullback-profile runner script."""
...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_speculative_volume_surge_prime_pullback_profile.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_speculative_volume_surge_prime_pullback_profile",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load speculative prime pullback-profile runner")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_speculative_volume_surge_prime_pullback_profile"] = module
    spec.loader.exec_module(module)
    return module


def test_parse_args_accepts_speculative_prime_pullback_profile_options() -> None:
    module = _load_module()
    ...
    assert args.primary_horizon == 20
    assert args.run_id == "20260420_180000_testabcd"
...
def test_main_runs_speculative_prime_pullback_profile_and_prints_payload(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    fake_bundle = SimpleNamespace(
        experiment_id="market-behavior/speculative-volume-surge-prime-pullback-profile",
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260420_180000_testabcd"
...
def test_main_module_executes_as_script(monkeypatch) -> None:
    ...
    import src.domains.analytics.speculative_volume_surge_prime_pullback_profile as research_module

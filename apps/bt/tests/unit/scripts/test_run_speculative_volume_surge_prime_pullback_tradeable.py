"""Tests for the tradeable prime pullback runner script."""
...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_speculative_volume_surge_prime_pullback_tradeable.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_speculative_volume_surge_prime_pullback_tradeable",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load speculative prime pullback tradeable runner")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_speculative_volume_surge_prime_pullback_tradeable"] = module
    spec.loader.exec_module(module)
    return module


def test_parse_args_accepts_tradeable_prime_pullback_options() -> None:
    module = _load_module()
    ...
    args = module.parse_args(
        [
            ...
            "/tmp/research",
            "--run-id",
            "20260420_200000_testabcd",
        ]
    )

    assert args.db_path == "/tmp/market.duckdb"
    assert args.lookback_years == 5
    assert args.pullback_search_window == 25
    assert args.entry_buckets == ("0-10%", "10-20%")
    assert args.holding_period_sessions == 15
    assert args.run_id == "20260420_200000_testabcd"
...
def test_main_runs_tradeable_prime_pullback_and_prints_payload(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    fake_bundle = SimpleNamespace(
        experiment_id="market-behavior/speculative-volume-surge-prime-pullback-tradeable",
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260420_200000_testabcd"
...
def test_main_module_executes_as_script(monkeypatch) -> None:
    ...
    import src.domains.analytics.speculative_volume_surge_prime_pullback_tradeable as research_module

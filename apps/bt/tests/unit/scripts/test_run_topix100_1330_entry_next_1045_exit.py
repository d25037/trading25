...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_topix100_1330_entry_next_1045_exit.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_topix100_1330_entry_next_1045_exit",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load run_topix100_1330_entry_next_1045_exit module")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix100_1330_entry_next_1045_exit"] = module
...
def test_main_runs_research_and_prints_bundle_payload(monkeypatch, capsys) -> None:
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260416_010500_testabcd"

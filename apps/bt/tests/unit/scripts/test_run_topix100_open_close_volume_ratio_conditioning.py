...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "run_topix100_open_close_volume_ratio_conditioning",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(
            "Failed to load run_topix100_open_close_volume_ratio_conditioning module"
        )
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix100_open_close_volume_ratio_conditioning"] = module

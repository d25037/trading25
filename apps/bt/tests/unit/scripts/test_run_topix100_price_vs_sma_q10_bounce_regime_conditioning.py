...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "run_topix100_price_vs_sma_q10_bounce_regime_conditioning",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(
            "Failed to load run_topix100_price_vs_sma_q10_bounce_regime_conditioning module"
        )
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix100_price_vs_sma_q10_bounce_regime_conditioning"] = module
...
def test_main_runs_regime_conditioning_and_prints_bundle_payload(
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260331_173500_testabcd"

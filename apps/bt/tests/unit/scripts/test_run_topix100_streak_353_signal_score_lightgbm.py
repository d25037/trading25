"""Tests for the TOPIX100 streak 3/53 score LightGBM runner."""
...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_topix100_streak_353_signal_score_lightgbm.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_topix100_streak_353_signal_score_lightgbm",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load TOPIX100 score LightGBM runner")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix100_streak_353_signal_score_lightgbm"] = module
    spec.loader.exec_module(module)
    return module


def test_parse_args_accepts_lightgbm_score_options() -> None:
    module = _load_module()
    ...
    assert args.long_target_horizon_days == 5
    assert args.short_target_horizon_days == 1
    assert args.top_k_values == [5, 10, 20]
    assert args.run_id == "20260406_220000_testabcd"
...
def test_main_runs_research_and_prints_bundle_payload(monkeypatch, capsys) -> None:
    ...
    fake_bundle = SimpleNamespace(
        experiment_id="market-behavior/topix100-streak-3-53-signal-score-lightgbm",
    ...
    exit_code = module.main(["--run-id", "20260406_220000_testabcd"])

    payload = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert payload["runId"] == "20260406_220000_testabcd"

...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_topix100_vi_change_regime_conditioning.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_topix100_vi_change_regime_conditioning",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(
            "Failed to load run_topix100_vi_change_regime_conditioning module"
        )
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix100_vi_change_regime_conditioning"] = module
...
def test_main_runs_vi_regime_and_prints_bundle_payload(
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260331_182500_testabcd"

...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "run_topix_downside_return_standard_deviation_exposure_timing",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(
            "Failed to load TOPIX downside return-standard-deviation runner module"
        )
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_topix_downside_return_standard_deviation_exposure_timing"] = module
    spec.loader.exec_module(module)
    return module


def test_parse_args_accepts_topix_downside_return_standard_deviation_runner_options() -> None:
    module = _load_module()
    ...
    assert args.high_annualized_downside_return_standard_deviation_thresholds == (0.15, 0.20)
    assert args.low_annualized_downside_return_standard_deviation_thresholds == (0.05, 0.10)

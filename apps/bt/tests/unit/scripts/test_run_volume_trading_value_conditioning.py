"""Tests for the volume-vs-trading-value conditioning runner script."""
...
def _load_module():
    ...
    module_path = (
        ...
        / "scripts"
        / "research"
        / "run_volume_trading_value_conditioning.py"
    )
    spec = importlib.util.spec_from_file_location(
        "run_volume_trading_value_conditioning",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("Failed to load volume-vs-trading-value runner module")
    module = importlib.util.module_from_spec(spec)
    sys.modules["run_volume_trading_value_conditioning"] = module
    spec.loader.exec_module(module)
    return module


def test_parse_args_accepts_volume_trading_value_conditioning_options() -> None:
    module = _load_module()
    ...
    assert args.analysis_use_sampled_codes is True
    assert args.short_windows == (20, 50)
    assert args.long_windows == (60, 150)
    assert args.thresholds == (1.2, 1.7)
    assert args.horizons == (5, 10)
    assert args.top_k == 2
    assert args.run_id == "20260414_180000_testcond"
...
def test_main_runs_volume_trading_value_conditioning_and_prints_bundle_payload(
    monkeypatch,
    capsys,
) -> None:
    module = _load_module()
    ...
    fake_bundle = SimpleNamespace(
        experiment_id="market-behavior/volume-trading-value-conditioning",
    ...
    assert exit_code == 0
    assert payload["runId"] == "20260414_180000_testcond"
...
def test_main_module_executes_as_script(monkeypatch) -> None:
    ...
    import src.domains.analytics.volume_trading_value_conditioning as research_module

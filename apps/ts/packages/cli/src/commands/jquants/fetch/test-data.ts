...
import { setupJQuantsClient } from '../helpers.js';
...
export async function fetchToyotaTestData(options: TestDataOptions): Promise<void> {
  ...
  try {
    const client = await setupJQuantsClient();
    ...
    const response = await client.getDailyQuotes(params);
    ...
    if (response.data && response.data.length > 0) {
      ...
      const filepath = await exporter.exportDailyQuotes(response.data, 'toyota_7203_daily.csv');

...
describe('shared logger', () => {
  ...
  test('logs in server mode using console log/warn/error routing', () => {
  ...
  test('ConsoleLogger delegates all methods to the shared singleton logger', () => {
  ...
  test('logs in browser mode using styled console output', () => {
  ...
  test('child logger inherits correlation id when context omits it', () => {
  ...
  test('detectBrowser catches and reports probe failures once outside test env', () => {
    ...
    const detectionWarnings = warnSpy.mock.calls.filter(([message]) =>
      String(message).includes('[shared logger] Browser environment detection failed')
    );

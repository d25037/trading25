...
export function OptimizationHtmlFileBrowser() {
  ...
  const [selectedFile, setSelectedFile] = useState<OptimizationHtmlFileInfo | null>(null);
  const [isRenaming, setIsRenaming] = useState(false);
  const [renameValue, setRenameValue] = useState('');
  const renameInputRef = useRef<HTMLInputElement>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  const { data: htmlFilesData, isLoading: isLoadingFiles } = useOptimizationHtmlFiles(
    selectedStrategy === 'all' ? undefined : selectedStrategy
  );
  const { data: htmlContent, isLoading: isLoadingContent } = useOptimizationHtmlFileContent(

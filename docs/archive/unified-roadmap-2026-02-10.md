...
作成日: 2026-02-06
最終更新: 2026-02-10（Phase 4 完了・root/packages 削除・README/ステータス整合を反映）
...
| 4 | パッケージ分離 | **完了**（4A-4D 完了、責務再配置と CI 段階実行を反映） | Medium | 4-6 週 |
...
**期間**: 4-6 週 | **リスク**: Medium | **状態**: 完了（4A-4D 完了）
**再ベースライン日**: 2026-02-09
**完了日**: 2026-02-10
...
- 2026-02-10: ルート `packages/` を削除し、`README.md`（root/`apps/ts`/`apps/bt`/`contracts`）と本ロードマップのステータス表記を現行構成へ整合化。
...
- 4A-4D の受け入れ条件はすべて充足し、Phase 4 の実装作業は完了。
- `root/packages` は撤去し、運用構成を `apps/ts/packages` + `apps/bt/src/lib` + `contracts/` に確定。
- 関連トラッキング Issue:
  - `ts-124`（`../issues/done/ts-124-phase4-ts-package-separation.md`）: status `done`（2026-02-10 クローズ）
  - `bt-026`（`../issues/done/bt-026-phase4-python-domain-package-split.md`）: status `done`（2026-02-10 クローズ）

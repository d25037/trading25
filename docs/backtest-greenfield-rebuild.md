...
- execution semantics が signal processing 側に漏れている。
  - 今回の `shared_config.execution_policy.mode=current_session_round_trip` のような仕様は、本来は「どの情報がいつ利用可能か」という availability model で扱うべきで、個別 signal の 1 日ラグとして散らすべきではない。
- worker 分離が不完全。
...
移行前の実装では、`shared_config.execution_policy.mode=current_session_round_trip` の正しさを保つために、same-day ではない entry signal を 1 日ラグさせる必要が出た。これは応急処置としては正しいが、理想形ではない。
...
- 当日寄り付きギャップ signal は「当日寄り直後に利用可能」
- fundamentals は「開示日時以後にのみ利用可能」

こうしておけば、engine が no-lookahead を強制できる。

現行 `bt-040` の途中実装では、この availability model を `CompiledStrategyIR.signals[].availability` として生成し、`SignalProcessor` の current-session ラグ判定は compiled availability を優先するようにした。あわせて `/api/signals/reference` でも signal ごとの availability profile を返し、authoring UI から `standard` / `next_session_round_trip` / `current_session_round_trip` の差を確認できるようにしている。

- signal processor 側で個別に `shift(1)` する必要がない
- same-day signal と prior-day signal を同じ run 内で安全に共存できる

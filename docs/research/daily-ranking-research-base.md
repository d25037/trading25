...
| SMA5 count long diagnostic | `market-behavior/ranking-sma5-count-long-evidence` | `sma5_above_count_5d` is not a standalone long selector. Same-day follow-up shows `0/1` can still have short 5D rebound, so do not use it as immediate liquidation. Inside `Deep Value + Long Hybrid Leadership + ATR20 Accel` style scaffolds, `0/1` is weaker on 20D/60D and can trigger `exit_watch`; `2/3` and `4/5` stay valid. `4/5` is continuation confirmation, not a hard filter; crowded long still needs low PBR / low Fwd PER value confirmation first. |

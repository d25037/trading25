...
- 2026-03-24: 再計測の結果、`tests/unit/server/routes -m 'not slow'` は `293 passed in 27.14s -> 4.69s`、`tests/unit/server/test_*.py -m 'not slow'` は `480 passed in 42.50s -> 7.45s` まで短縮した。残る上位 hotspot は `test_openapi.py`、`test_routes_db_sync.py`、`test_routes_portfolio_factor_regression.py`、`test_routes_portfolio.py` の one-time setup と、`test_routes_dataset.py` / `test_routes_db_sync.py` の一部 end-to-end call time へ寄っている。

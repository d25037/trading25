...
## 結果
- 2026-03-24: ``scripts/bt-pytest.sh`` を追加し、`BT_PYTEST_FAST=1` による `slow` 除外、`BT_PYTEST_DURATIONS` による durations 可視化、`BT_PYTEST_MARKEXPR` による任意の `-m` 条件指定を可能にした。``scripts/test-packages.sh`` と ``scripts/test-apps.sh`` から共通利用する形に整理した。
- 2026-03-24: ``apps/bt/docs/test-runtime.md`` を追加し、baseline hotspot、`slow` marker policy、full/fast 実行コマンド、`pytest-xdist` をまだ導入しない理由、今後の lightweight refactor targets を明文化した。
- 2026-03-24: J-Quants retry/backoff と worker timeout 系の人工待機を短縮し、runtime-heavy な検証だけを `slow` 管理へ寄せた。対象は ``apps/bt/tests/unit/server/clients/test_jquants_client.py``、``apps/bt/tests/unit/server/test_backtest_worker.py``、``apps/bt/tests/unit/server/test_lab_worker.py``、``apps/bt/tests/unit/server/test_optimization_worker.py``、``apps/bt/tests/unit/server/db/test_time_series_store.py``、``apps/bt/tests/unit/strategies/mixins/test_backtest_executor_mixin_paths.py``、``apps/bt/tests/unit/utils/test_indicators.py``。
- 2026-03-24: smoke baseline helper が `TRADING25_*` env を漏らして `tests/paths` / `tests/security` を壊していた cross-test state leakage を修正した。対象は ``apps/bt/tests/unit/test_collect_production_smoke_baseline.py``、``apps/bt/tests/paths/test_resolver.py``、``apps/bt/tests/security/test_security_validation.py``。
- 2026-03-24: 不変な app/schema/market fixture を module 共有に寄せ、server unit の起動コストを削減した。対象は ``apps/bt/tests/unit/server/routes/test_analytics_complex.py``、``apps/bt/tests/unit/server/test_openapi.py``、``apps/bt/tests/unit/server/test_routes_portfolio_performance.py``。

...
## 受け入れ条件
- [x] child issue が作成済みで、依存順が明示されている。
- [x] rank research / regime conditioning / fixture builder / naming cleanup / hedge decomposition の workstream が個別 issue として管理される。
- [x] program 完了時に、analytics research module 群の重複箇所と残課題が整理される。

## 実施内容
- [x] tracking issue として child issue の完了状態を管理する。
- [x] 実装中に設計変更が出た場合は child issue と依存関係を更新する。
- [x] core 抽出後に notebook / test / wrapper の責務を再点検する。
...
### 完了
- [x] `bt-050` TOPIX rank research core を shared module へ抽出
- [x] `bt-051` regime conditioning core を shared module へ抽出
- [x] `bt-052` analytics unit test の market DB fixture を共通化
- [x] `bt-053` decile / bucket naming と research wrapper interface を整理
- [x] `bt-054` oversized analytics module を責務別に分割
